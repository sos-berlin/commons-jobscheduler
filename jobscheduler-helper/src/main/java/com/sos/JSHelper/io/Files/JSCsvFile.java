 ...
 *
 * \details Die Klasse repräsentiert eine Csv-Datei und bietet die typischen
 * Operationen für eine solche Datei.
 *
 * \section JSCsvFile.intro_sec Introduction
 *
 * \section JSCsvFile.samples Some Samples
 *
 * \code private void TestCSVFile() throws Exception { String strFileName =
 * "data/sample.csv"; JSCsvFile objCsvFile = new JSCsvFile(strFileName); String
 * strField = null; objCsvFile.loadHeaders(); while (true) { strField =
 * objCsvFile.readCSVField();
 * 
 * if (strField == null) break;
 * 
 * if (strField.equalsIgnoreCase(JSCsvFile.END_OF_LINE)) System.out.print("\n");
 * else
 * 
 * System.out.print(strField + "="); } System.out.print("\n");
 * objCsvFile.close();
 * 
 * objCsvFile = new JSCsvFile(strFileName); while ( (strField =
 * objCsvFile.readCSVField("Text")) != null) { message(strField); }
 * objCsvFile.close();
 * 
 * } \endcode
 *
...
public class JSCsvFile extends JSTextFile {
    ...
    /** \change Donnerstag, 5. März 2009 EQCPN IGNORECOLS Zeilen mit falscher
     * Anzahl an Spalten überlesen */
    ...
    public String[] readCSVLine() throws IOException {
        ...
        if (list.isEmpty() || intColumnCount != intFieldCount && flgCheckColumnCount == true) {
            ...
            // return null;
            // }
            // TODO: Besser eine Exception werfen. Der Empfänger soll selbst
     ...
     * \details
     *
     * \code while (true) { strField = objCsvFile.readCSVField(); if (strField
     * == null) break; if (strField.equalsIgnoreCase(JSCsvFile.END_OF_LINE))
     * System.out.print("\n"); else System.out.print(strField + "="); } \endcode
     ...
     *
     *        Am Ende einer Zeile (wenn also alle Felder für die Zeile mit
     *        Werten gefüllt sind) ist diese Methode aufzurufen. Damit wird der
     *        bis dahin gepufferte Satz dann auf die Ausgabe geschrieben und es
     *        werden einige interne Felder zurückgesetzt.
     *
     * @return JSCsvFile
     * @throws Exception */
    @Override
     ...
     *        hinzugefügt. Zwischen den einzelnen Werten im Puffer wird ein
     *        Feldtrenner (ColumnDelimiter) eingefügt.
     *
     * @param pstrS Feldwert
     * @return JSCsvFile
     * @throws Exception */
     ...
     *
     *        Hier werden die Textbegrenzer (Standard ist doppelte
     *        Anführungszeichen '"'; siehe Option \c ColumnDelimiter) in den
     *        Text eingebaut, falls es gefordert (Option \c
     *        AlwaysSurroundFielJSithQuotes) oder durch den Text selbst
     *        erforderlich ist.
     *
     *        Beginnt der Text bereits mit dem Textbegrenzer, so werden alle
     *        Textbegrenzer verdoppelt und der gesamte Text nocheinmal in
     *        einfache Begrenzer eingeschlossen. Ist der Text leer, werden
     *        Textbegrenzer eingesetzt, auch dann nicht, wenn über die Option
     ...
     * <name>AlwaysSurroundFielJSithQuotes</name>
    ...
    /*
     * ! AlwaysSurroundFielJSithQuotes - AlwaysSurroundFielJSithQuotes Setter:
     * AlwaysSurroundFielJSithQuotes
    ...
    /*
     * --------------------------------------------------------------------------
     * - <method type="smcw" version="1.0"> <name>IgnoreValueDelimiter</name>
     * <title>Ignore Value Delimiter</title> <description> <para> Ignore Value
     * Delimiter </para> <para> Initial-Wert (Default) ist "false" (ohne
     * Anführungszeichen). </para> <mandatory>true</mandatory> </description>
     * <params> <param name="param1" type=" " ref="byref|byvalue|out" > <para>
     * </para> </param> </params> <keywords>
     * <keyword>WarehouseTransactions</keyword> <keyword>CSV</keyword>
     * <keyword>Converter</keyword> <keyword>Import</keyword> </keywords>
     * <categories> <category>Inbound</category> <category>CSV</category>
     * <category>Mapics</category> </categories> </method>
     * ----------------------
     * ------------------------------------------------------
     */
    /*
     * !
     * @brief Ignore Value Delimiter Ignore Value Delimiter
     */
    /*
     * ! IgnoreValueDelimiter - Ignore Value Delimiter Getter: Ignore Value
     * Delimiter Example:
    ...
    /*
     * ! IgnoreValueDelimiter - Ignore Value Delimiter Setter: Ignore Value
     * Delimiter
     ...
     * \details
     *
     * @version $Id$2009-03-05-EQCPN \return void
     *
