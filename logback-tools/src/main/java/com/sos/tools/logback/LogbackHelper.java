 ...
 *
 * To bind the JobScheduler logger to the JobSchedulerLogbackAppender it is necessary to set the log object in the
 * appender at runtime. To do this the method prepareLogbackAppender() has to be called.
 *
 * To test if JobScheduler installation is configured well to use the Appender use isLogbackConfigured().
 *
...
public class LogbackHelper {
    ...
    /**
     * Bind the logger object of JobScheduler to the appender.
     * It is required that the logback configuration has a root logger referencing the JobSchedulerLogbackAppender.
     *
     * @param spooler_log
     */
    ...
    public static boolean isLogbackConfigured() {
        try {
            Class.forName("ch.qos.logback.classic.LoggerContext");		// runs with logback classic ?
        } catch( ClassNotFoundException e ) {
            logger.debug("Could not find class 'ch.qos.logback.classic.Logger' - Logback classic is not in classpath.");
            return false;
        }
        try {
            Class.forName("ch.qos.logback.core.Appender");		// runs with logback core ?
        } catch (ClassNotFoundException e) {
