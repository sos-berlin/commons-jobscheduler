...
public class SOSSSHJob2JSAdapter extends SOSSSHJob2JSBaseAdapter {
  ...
  private Map prefixSchedulerEnvVars(Map<String, String> allEnvVars){
	  Map envVars = new HashMap<String, String>();
//	  JTIL-224
...
//    					String nodeName = key.substring(0, currentNodeName.length() - 1);
    				...
    				} else if (Pattern.compile("\\W").matcher(key).find()){ // is not active AND contains special characters
  ...
  private boolean isActiveNodeParam(String key, String currentNodeName){
	...
	return false;  
  	...
  	@Override //TODO remove if process is reviewed and fixed! Original in JobSchedulerJobAdapter puts duplicated key/Value pair
