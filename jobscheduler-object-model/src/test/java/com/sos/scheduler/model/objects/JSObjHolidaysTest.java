...
public class JSObjHolidaysTest extends TestBase {
    ...
    public final void testWeekdays() {
        xml = "<run_time>" + "<holidays>" + "<weekdays>" + "<day day=\"6\"/>" + "<day day=\"7\"/>" + "</weekdays>" + "</holidays>" + "</run_time>";
    ...
    public final void testHolidays() {
        xml = "<run_time>" + "<holidays>" + "<holiday date=\"2012-03-31\"/>" + "<holiday date=\"2012-04-01\"/>" + "</holidays>" + "</run_time>";
        JSObjRunTime runtime = new JSObjRunTime(factory, xml);
    ...
    public final void testStartDatesAwareHolidays() {
        xml = "<run_time>" + "<holidays>" + "<holiday date=\"2012-03-31\"/>" + "</holidays>" + "</run_time>";
        JSObjRunTime runtime = new JSObjRunTime(factory, xml);
    ...
    /** \brief Test the definition of holidays via an include file (live_file
     * Attribute) \detail The order holidays-test contains an include for
     * defining holidays at the weekend (Saturday and Sunday). To call the
     * method getJsObjHolidays() of the runtime object will resolve the include
     * implicitely and therefore we can get the holidays as usaul via the
     * holidays object. To run this test it is necessary that the folder
     * testdata is declared as source folder
     * 
     * @throws IOException */
    @Test
    ...
    public final void testIncludeLive() throws IOException {
        ...
        // the order holidays_test contains a relative reference to the inlcude
        // holidays.xml in the root of the live folder
        testInclude(includeFactory, getResource("include/holidays-test.order.xml"));

        // the order weekdays_test contains a relative reference to the inlcude
        // weekdays.xml in the same directory
        testInclude(includeFactory, getResource("include/weekdays-test.order.xml"));

        // the absolute-file-test contains a absolute reference to the inlcude
        // weekdays.xml using an environment variable
    ...
    /*
     * To run this test it is necessary that the folder testdata is declared as
     * source folder
     */
    @Ignore("Test set to Ignore for later examination")
    public final void testIncludeLiveInvalid() throws MalformedURLException {
        ...
        // the relative-file-test contains a relative reference to the inlcude
        // weekdays.xml using an environment variable
    ...
    private final void testInclude(final SchedulerObjectFactory factory, final String configurationFile) {
        ...
        assertEquals(configurationFile, PathResolver.normalizePath(runtime.getHotFolderSrc().getName()));  // runtime
                                                                                                          // is
                                                                                                          // based
                                                                                                          // on
                                                                                                          // the
