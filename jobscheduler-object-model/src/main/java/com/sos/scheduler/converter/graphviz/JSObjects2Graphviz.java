...
public class JSObjects2Graphviz extends JSJobUtilitiesClass<JSObjects2GraphvizOptions> {
    ...
    public String createGraphvizFile(JSObjBase obj, List<JSObjBase> sortedFileList) throws Exception {
        ...
        if (obj instanceof JSObjJobChain) {
            ...
            objDotFile.WriteLine(AddQuotes(conPseudoNodeSTART) + " [label = " + AddQuotes(conPseudoNodeSTART + ": " + strName) + ", shape = "
                    + AddQuotes("box") + ", style = " + AddQuotes("solid") + "];");
            ...
            for (Object objO : objChain.getJobChainNodeOrFileOrderSinkOrJobChainNodeEnd()) {
                if (objO instanceof JobChainNode) {
                    ...
                    // else {
                    // objDotFile.WriteLine(AddQuotes(strState) + " -> " +
                    // AddQuotes("end"));
                    // }
                }
            }
            ...
            for (Object objO : objChain.getJobChainNodeOrFileOrderSinkOrJobChainNodeEnd()) {
                if (objO instanceof JobChainNode) {
                    ...
                    if (strErrorState != null && strErrorState.equalsIgnoreCase(strNextState) == false) {
                        objDotFile.WriteLine(AddQuotes(strState) + " -> " + AddQuotes(strErrorState)
                                + " [label=\" error\", fontcolor=\"grey\", fontsize=7, style=\"dotted\", constraint=false]");
