 ...
 * \details
 *
 * This junit-test will add two jobs to the current jobscheduler instance. First
 * job ist a Java-API job and the second is a java-script driven job.
 *
 * There is no checking in this junit-test wether the jobs exists. If the job
 * already exist it will be overwritten without any further notice.
 *
 * \return void
 *
 * \code
 * 
 * @Test public final void testCreateAddJobs() { JSObjJob objJavaJob =
 *       objFactory.createJob(); objJavaJob.setName("checkIfWritable");
 *       objJavaJob.setOrder(false);
 * 
 *       Description objDesc = objFactory.createJobDescription(); Include
 *       objDescInclude = objFactory.createInclude();
 ...
 * 
 *       Params objParams = objFactory.setParams(new String[] { "file",
 *       "jobs/JobSchedulerCanWrite.xml" }); objJavaJob.setParams(objParams);
 * 
 *       // javascript job JSObjJob objJavaScriptJob = objFactory.createJob();
 *       objJavaScriptJob.setName("hello"); objJavaScriptJob.setOrder(false);
 * 
 ...
 *       objAJ.getJob().add(objJavaJob); objAJ.getJob().add(objJavaScriptJob);
 * 
 *       objAJ.run(); try { objAJ.getAnswerWithException(); } catch
 *       (JSCommandOKException e) { // TODO: show added jobs } catch
 *       (JSCommandErrorException e) { fail(e.getMessage()); } } \endcode
 *
 ...
 *       </p>
 *       \author oh
 * @version $Id$03.02.2011 \see reference
 *
...
public class JSObjJob extends Job {
    ...
    public Script getScript() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::getScript";
        if (script == null) {
    ...
    public boolean getEnabled() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::name";
    ...
    public boolean isOrderDrivenJob() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::isOrderDrivenJob";
        if (order == null) {
            return false;
        } else {
    ...
    public Process getProcess() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::getProcess";
    ...
    public String getJobNameAndTitle() {
        ...
        if (this.isDisabled() == true) {
            // TODO i18n
            // strT += " (" +
            // sos.scheduler.editor.app.Messages.getLabel("disabled") + ")";
            strT += " (disabled)";
        }
    ...
    public boolean isDisabled() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::isDisabled";
        return false;
