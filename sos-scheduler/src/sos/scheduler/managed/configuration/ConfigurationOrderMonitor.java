...
public class ConfigurationOrderMonitor extends ConfigurationBaseMonitor {
	...
	public boolean spooler_process_before() {
		...
		try { // to map order configuration to this job
			...
			// Vielleicht besser RegEx ^(?:<\?[^?]+\?>)?\s*<order[^>]+\bjob_chain\s*=\s*["']([^"']+)["'].* statt SOSXMLPath
			// weil man sonst ganz viel Zeug hat (Tonnen von Log-Zeilen), die nicht gebraucht werden
			//JITL-82
			...
			if (order.params().value(conParamNameCONFIGURATION_PATH) != null && order.params().value(conParamNameCONFIGURATION_PATH).length() > 0) {
				...
				if (spooler_task.params().value(conParamNameCONFIGURATION_PATH) != null && spooler_task.params().value(conParamNameCONFIGURATION_PATH).length() > 0) {
			...
			if (order.params().value(conParamNameCONFIGURATION_FILE) != null && order.params().value(conParamNameCONFIGURATION_FILE).length() > 0) {
				...
				if (spooler_task.params().value(conParamNameCONFIGURATION_FILE) != null && spooler_task.params().value(conParamNameCONFIGURATION_FILE).length() > 0) {
