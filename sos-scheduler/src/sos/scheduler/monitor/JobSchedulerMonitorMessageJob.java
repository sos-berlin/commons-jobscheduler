			...
			try { // to fetch parameters from the job and from orders that have precedence to job parameters
                ...
                if (sosString.parseToString(spooler_task.params().var("maximum_reports")).length() > 0) {
                    try {
                        ...
                        setMaximumReports(-1);
                        spooler_log.debug1("..job parameter [maximum_reports=" + 
                                sosString.parseToString(orderData.var("maximum_reports")) + "] is not valid cause: " + e.getMessage() );
                    }
                }  else {
                    setMaximumReports(-1);
                }
                ...
                if (spooler_task.job().order_queue() != null) {
					...
					if (order != null) {
						...
						if ( orderData.var("persistent") != null && orderData.var("persistent").toString().length() > 0) {
							if (orderData.var("persistent").equals("0") 
									|| orderData.var("persistent").equalsIgnoreCase("false")
						...
						if(orderData != null && sosString.parseToString(orderData.var("maximum_reports")).length() > 0) {
							try {
								...
								setMaximumReports(-1);
								spooler_log.debug1("..order parameter [maximum_reports=" + 
										sosString.parseToString(orderData.var("maximum_reports")) + "] is not valid cause: " + e.getMessage() );
							}
						} else {
							setMaximumReports(-1);
...
//				} else {					
					...
					while(messageIterator.hasNext()) {
						...
						if (spooler_task.job().order_queue() != null) {
							if (message.get("WARN") != null) {
								countWarnings++;
								xml_payload += "<message id='" + countMessages + "' severity='WARN'><![CDATA[" + this.normalizeItem(message.get("WARN").toString()).substring(0, message.get("WARN").toString().indexOf("[report_counter=")) + "]]></message>";
							} else if (message.get("ERROR") != null) {
								countErrors++;
								xml_payload += "<message id='" + countMessages + "' severity='ERROR'><![CDATA[" + this.normalizeItem(message.get("ERROR").toString()).substring(0, message.get("ERROR").toString().indexOf("[report_counter="))  + "]]></message>";
							} else if (message.get("info") != null) {
								countInfos++;
								xml_payload += "<message id='" + countMessages + "' severity='info'><![CDATA[" + this.normalizeItem(message.get("info").toString()) + "]]></message>";
							}
						} else {
							if (message.get("WARN") != null) {
								countWarnings++;
						...
						if (isMessagePersistent()) {
							if (message.get("WARN") != null && message.get("WARN").toString() != null) {
								this.messageVariables.add("<message id='" + countMessages + "'><severity>WARN</severity><content>" + this.normalizeItem(message.get("WARN").toString()) + "</content></message>");
							} else if (message.get("ERROR") != null && message.get("ERROR").toString() != null) {
								this.messageVariables.add("<message id='" + countMessages + "'><severity>ERROR</severity><content>" + this.normalizeItem(message.get("ERROR").toString()) + "</content></message>");
							}
						}
					}
			...
			try { // to update scheduler variables for persistent warnings and errors
	 ...
	 * Beispiel 1:
	 * 
	 * 2007-02-16 14:29:26.671 [WARN]   (Job  monitor_check_status) ERRNO-2  No such file or directory [stat] [c:/scheduler.hobbit/jobs/JobSchedulerMonitorMessageJob.xml]}
	 *      <==>
	...
	private String transformString(HashMap h) throws Exception {
		...
		try {		
			...
			if(currStr.startsWith("[order=")) {
				...
				retVal = "[Timestamp: " + currStr.substring(startTS, endTS ) + "] " +"["+ logtype + "] " +
				"[Job Chain:" + currStr.substring(startOrdername, endOrdername ) + ", " +
				//"Order ID: " + currStr.substring(endOrdername + 1, startTS -1  )  + ", " +
	...
	private HashMap getIgnoreMessages() throws Exception {
		...
		try {
			iMessages = (!includeSupervisor ? sosString.parseToString(includeSupervisorMsg)+ ";" : "") 
			+ sosString.parseToString(excludeMessages);
			...
			if(getMaximumReports() > -1) {
				...
				if (curContent.indexOf("[report_counter=") > -1) {
					...
					if(counter > getMaximumReports()) {
						spooler_log.info("..the following warnings and errors were dropped due to maximum_reports constrains (" + getMaximumReports() + ") for [logline="+ curContent + "]");					
						return true;
					} 
				}
			}
