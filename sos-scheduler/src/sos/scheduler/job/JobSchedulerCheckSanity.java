 ...
 * 
 * <p>Die Einstellungen teilen sich auf in Kategorien mit Unterkategorien.
 * Zu jedem Kategorie-Unterkategorie Paar wird ein Wert (Value) angegeben.</p>
 * 
...
//        	 initialize hostware
            ...
            if (spooler_log != null) try { spooler_log.error(e.getMessage()); } catch (Exception ex) {}
            return false;
        }
    ...
    public boolean spooler_process() {
        ...
        try {         
            	...
            	if(checkDiskSpace) checkDiskSpace(false);
            	if(checkDiskSpaceUser) checkDiskSpace(true);
            	if(checkMemorySize) checkMemory();         	
            ...
            while (keys.hasMoreElements()){
            	...
            	if (key.startsWith("category_")){
            		...
            		if(category.equalsIgnoreCase("free_memory")){
            			Long minMem = new Long(minMemorySize);
                    	try{minMem = new Long(value);} catch  (Exception e){}
                    	if (subtype.length()>0){
    ...
    private void checkDiskSpace(boolean user) throws Exception{
        ...
        while (iter.hasNext()){        	
            ...
            if (freeDiskSpace <= minDS) {
            	String message="free disk space on partition "+location+sUser+" has fallen below minimum value [" + 
                minDS/1048576 + "MB]: " + freeDiskSpace/1048576 + "MB" ;
                if (spooler_log != null) spooler_log.error(message);
                writeHistory(historyKey,location,""+freeDiskSpace,""+minDS,
                		true,message);
            } else {
                if (spooler_log != null) spooler_log.info("minimum disk space on partition "+location+sUser+": " + 
                    minDS/1048576 + "MB" +
                    ", free disk space"+sUser+": " + freeDiskSpace/1048576 + "MB");
                writeHistory(historyKey,location,""+freeDiskSpace,""+minDS,
                		false,"");
            }
        }
    ...
    private void checkMemory() throws Exception{
        ...
        while (iter.hasNext()){
            ...
            if (freeMemory <= minMem) {
            	String message="free memory ["+type+"] has fallen below minimum value [" + 
                minMem/1048576 + "MB]: " + freeMemory/1048576 + "MB" ;
                if (spooler_log != null) spooler_log.error(message);
                writeHistory("free_memory",type,""+freeMemory,""+minMem,
                		true,message);
            } else {
                if (spooler_log != null) spooler_log.info("minimum memory ["+type+"]: " + 
                    minMem/1048576 + "MB" +
    ...
    private void writeHistory(String category, String subType, String value, String refValue, 
    		boolean failed, String message){
    	if (this.getConnection()==null) return; //No Database Settings when running without Database
    	String currentStateText="";
    	if (message!=null) currentStateText = message;
    	...
    	int id=0;
    	if (spooler_task != null) id= spooler_task.id();
    	String sFailed="0";
    	if (failed) sFailed="1";
    	...
    	String query="INSERT INTO SCHEDULER_SANITY_HISTORY (\"SPOOLER_ID\", \"JOB_ID\"," +
    			...
    			" \"MESSAGE\", \"CREATED\", \"CREATED_BY\") VALUES (" +
    			"'"+spooler.id()+"', '"+id+"', '"+category+"', '"+subType+
    			"', '"+value+"', '"+refValue+"', "+sFailed +", '"+currentStateText+
				"', %now, '"+spooler_job.name()+"')";
    	try{
