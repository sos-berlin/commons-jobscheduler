...
/**
 * post files via http, based on Jakarta Commons HTTP Client, see commons-httpclient-3.0.jar
 * 
 * job parameters:
 * input_directory
 * input_filespec
...
public class JobSchedulerHttpPostJob extends JobSchedulerJob {
    ...
    public boolean spooler_open() {
        ...
        try {
            if (this.getInputDirectory() == null || this.getInputDirectory().length() == 0) return true;
            
            File inputFile = new File(this.getInputDirectory());
            if (inputFile.isDirectory()) {
                if (!inputFile.canRead()) throw new Exception("input directory is not accessible: " + inputFile.getCanonicalPath());
                ...
                if (!inputFile.canRead()) throw new Exception("input file is not accessible: " + inputFile.getCanonicalPath());
                this.inputFileList = new Vector();
                this.inputFileList.add(inputFile);
            }

            if (this.inputFileList.size() > 0) spooler_log.info(this.inputFileList.size() + " input files found");
            
            this.inputFileListIterator = this.inputFileList.iterator();            
            return this.inputFileListIterator.hasNext();
            
        } catch (Exception e){
            spooler_log.warn("failed to retrieve input files from directory [" + this.getInputDirectory() + ", " + this.getInputFileSpec() + "]: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean spooler_process() {   
        ...
        try {
            ...
            if (url == null || url.length() == 0) url = this.getUrl();
            if (inputFileSpec == null || inputFileSpec.length() == 0) inputFileSpec = this.getInputFileSpec();

            if (!inputFile.exists()) throw new Exception("input file [" + inputFile.getCanonicalPath() + "] does not exist.");
            if (url == null || url.length() == 0) throw new Exception("no URL was given to post files.");
            
            
            if (contentType == null || contentType.length() == 0) contentType = this.getContentType();
            if (contentType == null || contentType.length() == 0) {
                ...
                if ( contentType.equals("text/html") || contentType.equals("text/xml") ) {
                    ...
                    if ( m.find() ) contentType += "; charset="+m.group(1);
                    br.close();
                }
            }
            ...
            if (inputFile != null) message += " file [" +  inputFile.getAbsolutePath() + "]";
            if (url != null && url.length() > 0) message += " for url [" + url + "]"; 
    ...
    private void logResponse(File inputFile, File outputFile, InputStream responseStream) throws Exception {
        ...
        if (outputFile.isDirectory()) outputFile = new File(outputFile.getCanonicalPath() + "/" + inputFile.getName());
