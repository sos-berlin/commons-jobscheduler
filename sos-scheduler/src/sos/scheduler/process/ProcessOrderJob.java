...
/** 
 * <p>ProcessOrderJob ist die Basisklasse für auftragsgesteuerte Jobs und stellt Methoden für Jobs zur Verfügung:</p>
 * <p>- spooler_init(): stdout und stderr als streams lesen</p>
 * <p>- spooler_exit(): cleanup</p>
 * <p>- executeSubprocess(): Subprozesse ausführen</p>
 * <p>- prepare(), prepareParameters(), prepareAttributes(): Parameter des Jobs und des Auftrags zusammenführen</p>
 * <p>- initConfiguration(): XML-Konfigurationsdatei als XML payload an den Auftrag übergeben</p>
 * <p>- prepareConfiguration(): liefert die XML-Konfiguration des Auftrags als Auftragsparameter und stellt ein DOM zur Verfügung.</p>
 * 
...
public class ProcessOrderJob extends ProcessJob { 
    ...
    public void prepareAttributes() throws Exception {
        ...
        try { // to set processing attributes from parameters
            ...
            if (this.getParameters().value("scheduler_order_command_parameters") != null && this.getParameters().value("scheduler_order_command_parameters").length() > 0) {
            ...
            if (this.getParameters().value("scheduler_order_priority_class") != null && this.getParameters().value("scheduler_order_priority_class").length() > 0) {
    ...
    public  String normalizedPath(String path, boolean addSeparator) throws Exception {
        ...
        if (addSeparator) {
            if (!path.endsWith("/") && !path.endsWith("\\")) path = path + File.separatorChar;
        } else {
