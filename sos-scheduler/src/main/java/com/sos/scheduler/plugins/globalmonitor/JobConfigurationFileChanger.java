...
public class JobConfigurationFileChanger {
    ...
    /*
     * private static Document convertJdomDocument2W3Document(org.jdom.Document
     * jdomDoc) throws IOException { DOMOutputter domOut = new DOMOutputter();
     * try { return domOut.output(jdomDoc); } catch (JDOMException jde) { throw
     * new IOException("JDOM output exception"); } } public Document
     * getJobAsDocument() throws JAXBException, ParseException,
     * FileNotFoundException { if (job != null) { DOMResult res = new
     * DOMResult(); JAXBContext context = JAXBContext.newInstance(Job.class);
     * context.createMarshaller().marshal(job, res); Document doc = (Document)
     * res.getNode(); return doc; } return null; } public Job
     * readConfigurationFile() throws JAXBException, FileNotFoundException {
     * JAXBContext context = JAXBContext.newInstance(Job.class); Unmarshaller
     * unmarshaller = context.createUnmarshaller(); job = (Job)
     * unmarshaller.unmarshal(jobfileToChange.getConfigurationFile()); return
     * job; } public Job readConfigurationFile(Document e) throws JAXBException,
     * FileNotFoundException { JAXBContext context =
     * JAXBContext.newInstance(Job.class); Unmarshaller unmarshaller =
     * context.createUnmarshaller(); job = (Job) unmarshaller.unmarshal(e);
     * return job; } public void changeConfiguration() throws JAXBException,
     * ParseException, IOException { Document jobDocument = getJobAsDocument();
     * logger.info("---------- CHANGE"); job =
     * readConfigurationFile(addMonitorUse(jobDocument)); } public void
     * writeFile() throws JAXBException, ParseException, FileNotFoundException {
     * if (job != null) { JAXBContext context =
     * JAXBContext.newInstance(Job.class); Marshaller m =
     * context.createMarshaller();
     * m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
     * m.marshal(job, new
     * FileOutputStream(jobfileToChange.getConfigurationFile())); } }
     */
