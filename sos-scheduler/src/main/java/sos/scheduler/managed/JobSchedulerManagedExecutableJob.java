 ...
 * 
 * order parameters: ignore_error (optional): bei true werden Fehler des
 * executables ignoriert ignore_signal (optional): bei true werden
 * Rückgabesignale des executables ignoriert ignore_stderr (optional): bei true
 * werden Zeichen in stdout nicht als Fehler behandelt timeout (optional):
 * timeout für die Ausführung des executables in Sekunden log_file (optional):
 * Der Scheduler nimmt nach Beenden des Subprozesses den Inhalt dieser Datei in
 * sein Protokoll
 * 
 * environment variablen können über parameter mit dem Präfix env. gesetzt
 * werden, z.B.: env.LD_LIBRARY_PATH
 * 
...
public class JobSchedulerManagedExecutableJob extends JobSchedulerManagedJob {
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            // if ( orderData == null || orderData.var("command") == null ||
            // orderData.var("command").toString().length() == 0)
            // throw (new
            // Exception("no executable command found in order payload"));
            ...
            if (orderPayload != null) {
                ...
                if (orderPayload.var("ignore_error") != null
                        && (orderPayload.var("ignore_error").toString().equalsIgnoreCase("true") || orderPayload.var("ignore_error").equalsIgnoreCase("1") || orderPayload.var("ignore_error").equalsIgnoreCase("yes"))) {
                    ignoreError = true;
                }
                if (orderPayload.var("ignore_signal") != null
                        && (orderPayload.var("ignore_signal").toString().equalsIgnoreCase("true") || orderPayload.var("ignore_signal").equalsIgnoreCase("1") || orderPayload.var("ignore_signal").equalsIgnoreCase("yes"))) {
                    ignoreSignal = true;
                }
                if (orderPayload.var("ignore_stderr") != null
                        && (orderPayload.var("ignore_stderr").toString().equalsIgnoreCase("true") || orderPayload.var("ignore_stderr").equalsIgnoreCase("1") || orderPayload.var("ignore_stderr").equalsIgnoreCase("yes"))) {
                    ignoreStderr = true;
                }
                if (orderPayload.var("ignore_timeout") != null
                        && (orderPayload.var("ignore_timeout").toString().equalsIgnoreCase("true") || orderPayload.var("ignore_timeout").equalsIgnoreCase("1") || orderPayload.var("ignore_timeout").equalsIgnoreCase("yes"))) {
                    ignoreTimeout = true;
                }
            ...
            /*
             * if(orderData!=null && orderData.var("environment")!=null &&
             * orderData.var("environment").toString().length() > 0){ String
             * environment=orderData.var("environment").toString(); Pattern
             * envPattern = Pattern.compile("\\{[^\\}]*\\}"); Matcher envMatcher
             * = envPattern.matcher(environment);
             * spooler_log.debug3("environment variables:");
             * while(envMatcher.find()){ String nameValueBr=envMatcher.group();
             * String nameValue=nameValueBr.substring(1,nameValueBr.length()-1);
             * spooler_log.debug3("  "+nameValue); String[]
             * nameValueArr=nameValue.split("="); if (nameValueArr.length!=2){
             * spooler_log.warn("Incorrect Syntax for Name=Value Pair.");
             * continue; } subProc.set_environment(nameValueArr[0],
             * nameValueArr[1]); } }
             */
