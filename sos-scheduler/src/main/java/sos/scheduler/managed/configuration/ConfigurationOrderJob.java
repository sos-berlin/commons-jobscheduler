...
public class ConfigurationOrderJob extends ConfigurationJob {
    ...
    public boolean spooler_init() {
        try {
            ...
            try {
                if (spooler_task != null && spooler_task.stdout_path() != null && spooler_task.stdout_path().isEmpty() && spooler_task.stderr_path() != null
                        && !spooler_task.stderr_path().isEmpty()) {
    ...
    public Subprocess executeSubprocess(final String command, final String commandParameters, final HashMap environment) throws Exception {
        ...
        try {
            ...
            for (String parameterName : parameterNames) {
                commandLine = myReplaceAll(commandLine, "\\$\\{" + parameterName + "\\}",
                        this.getParameters().value(parameterName).replaceAll("[\\\\]", "\\\\\\\\"));
            }
    ...
    public void prepareAttributes() throws Exception {
        try {
            ...
            if (this.getParameters().value("scheduler_order_ignore_stderr") != null && !this.getParameters().value("scheduler_order_ignore_stderr").isEmpty()) {
            ...
            if (this.getParameters().value("scheduler_order_ignore_error") != null && !this.getParameters().value("scheduler_order_ignore_error").isEmpty()) {
            ...
            if (this.getParameters().value("scheduler_order_ignore_signal") != null && !this.getParameters().value("scheduler_order_ignore_signal").isEmpty()) {
    ...
    public void initConfiguration(final String configurationPath, final String configurationFilename) throws Exception {
        if (spooler_job.order_queue() != null && spooler_task.order().params() != null && spooler_task.order().params().value("configuration_path") != null
