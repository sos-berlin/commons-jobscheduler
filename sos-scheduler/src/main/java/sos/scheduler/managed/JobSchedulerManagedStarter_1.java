...
public class JobSchedulerManagedStarter_1 extends JobSchedulerJob {
    ...
    /** Settings Attribut: maxOrderCount: max. Größe der Auftragswarteschlangen
     * aller Job-Ketten */
        ...
        try {
            ...
            if (!startscript && (order + model + job + once).length() > 0) {
                if (order.length() > 0)
                    ...
                    if (!initJobChains(model, remove.equalsIgnoreCase("true")))
    ...
    public void initJobs(String jobID, boolean once, String model) throws Exception {
        ...
        try { // to retrieve jobs
            ...
            while (it.hasNext()) {
                ...
                // if (
                // this.spooler.job_exists(currJob.get("job_name").toString()))
                // continue;
                ...
                // if(currJob.get("spooler_id")!=null)
                // jobElement.setAttribute("spooler_id",
                // currJob.get("spooler_id").toString());
                ...
                String jobParams = this.getConnection().getClob("SELECT \"PARAMS\" FROM " + JobSchedulerManagedObject.getTableManagedJobs() + " WHERE \"ID\"="
                        + currJob.get("id"));
                ...
                String jobScript = this.getConnection().getClob("SELECT \"SCRIPT\" FROM " + JobSchedulerManagedObject.getTableManagedJobs() + " WHERE \"ID\"="
                        + currJob.get("id"));
                ...
                try { // darf schiefgehen, weil evtl Monitor Feld nicht
                      // vorhanden ist
                ...
                if (jobMonitor == null || jobMonitor.length() == 0) {
                    ...
                    // if(!validateMonitor(jobMonitor,"Job "+currJob.get("job_name")))
                    // continue;
                    ...
                    if (script != null && script.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (language.length() > 0) {
                            ...
                            // String startScript =
                            // this.getConnectionSettings().getSectionEntry("scheduler","script","monitor_start_script."+language.toLowerCase());
                            String startScript = this.getConnection().getClob("SELECT \"LONG_VALUE\" FROM " + JobSchedulerManagedObject.getTableSettings()
                                    + " WHERE \"APPLICATION\"='scheduler' " + "AND \"SECTION\"='script' AND \"NAME\"='" + "monitor_start_script."
                ...
                if (jobRuntime != null && jobRuntime.length() > 0) {
                    ...
                    /*
                     * if
                     * (!runtimeElement.getAttribute("once").equalsIgnoreCase(
                     * "yes")) { runtimeElement.setAttribute("once","yes"); }
                     */
                ...
                /*
                 * String jobContent =
                 * this.getConnection().getClob("SELECT \"JOB\" FROM " +
                 * JobSchedulerManagedObject.getTableManagedJobs() +
                 * " WHERE \"ID\"=" + currJob.get("id")); if (jobContent == null
                 * || jobContent.length() == 0) { if (this.getLog() != null)
                 * this
                 * .getLog().warn("no job configuration found for managed job: "
                 * + currJob.get("id")); continue; }
                 */
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    if (modelId.length() == 0) {
                        ...
                        // wenns die schon gibt, false zurückgeben, damit
                        // bekannt ist,
                    ...
                    /*
                     * if (
                     * this.spooler.job_chain_exists(currWorkflowModel.get("name"
                     * ).toString())){ jobChain =
                     * spooler.job_chain(currWorkflowModel
                     * .get("name").toString()); } else
                    ...
                    if (currJobs.isEmpty()) {
                        if (this.getLog() != null)
                            this.getLog().warn(".. no job found for this workflow model [" + currWorkflowModel.get("id").toString() + "]: "
                    ...
                    while (itJobs.hasNext()) {
                        ...
                        if (currJob.containsKey("job_name") && currJob.containsKey("input_level") && currJob.containsKey("output_level")
                            ...
                            /*
                             * if (Integer.parseInt(currJob.get("output_level").
                             * toString()) > maxOutputLevel) maxOutputLevel =
                             * Integer.parseInt(currJob.get("output_level").
                             * toString());
                             */

                            jobChain.add_job(currJob.get("job_name").toString(), currJob.get("input_level").toString(), currJob.get("output_level").toString(), currJob.get("error_level").toString());
                            ...
                            if (this.getLog() != null)
                                this.getLog().debug3(".. job_name=" + currJob.get("job_name").toString() + ", input_level="
                                        + currJob.get("input_level").toString() + ", output_level=" + currJob.get("output_level").toString() + ", error_level="
                                        + currJob.get("error_level").toString() + " .. added.");

                        } // if
                    } // while
                      // Endzustände für Fehlerzustände ohne Folgeverarbeitung
                      // bekanntgeben:
                    ...
                    // Endzustand des letzten Jobs mit positiven Status
                    // bekanntgeben
                    /*
                     * if (this.getLog() != null) this.getLog().debug9(
                     * ".. try to add_end_state with maxOutputLevel: " +
                     * Integer.toString(maxOutputLevel));
                     * jobChain.add_end_state(Integer.toString(maxOutputLevel));
                     * if (this.getLog() != null) this.getLog().debug9(
                     * ".. .. add_end_state with with maxOutputLevel: " +
                     * Integer.toString(maxOutputLevel) + " ok.");
                     */

                    // Ausganszustände, die nicht als Eingangszustand auftauchen
                    // als Endzustand bekanntgeben:
                    ...
                    // Job Chain ist evtl. in parallelem Job-Lauf angelegt
                    // worden
    ...
    public boolean initOrders(String orderID) {
        ...
        try {
            ...
            while (this.getOrderIterator().hasNext()) {
                ...
                // do not add orders if job chain exceeds setting for order
                // queue length
                if (maxOrderCount > 0) {
                    if (spooler.job_chain(rec.get("name").toString()).order_count() >= this.getMaxOrderCount()) {
                        this.getLog().info(".. current order for workflow [" + currentWorkflowOrderId + "] skipped: orderQueueLength ["
                ...
                if (rec.get("title") != null && rec.get("title").toString().length() > 0) {
                    ...
                    order.set_title(rec.get("order_id").toString());   // Auftragstitel
                ...
                try { // to create order payload
                    ...
                    String payload = this.getConnection().getClob("SELECT \"PARAMS\" FROM " + JobSchedulerManagedObject.getTableManagedOrders()
                            + " WHERE \"SPOOLER_ID\"" + ((spoolerId != null && spoolerId.length() > 0) ? "='" + spoolerId + "'" : " IS NULL")
                            + " AND \"JOB_CHAIN\"='" + rec.get("job_chain").toString() + "'" + " AND \"ORDER_ID\"='" + rec.get("order_id").toString() + "'");
                ...
                this.getLog().info("order [" + currentWorkflowOrderId + "] added to workflow [" + currentWorkflowModelId + "] " + "with level ["
                        + currentJobLevel + "]: " + currentWorkflowModelName);
            } // while
    ...
    public HashMap getCurrentJob(int workflowModel, int taskLevel) throws Exception {
        ...
        try {
            query = new StringBuffer("SELECT \"TITLE\", \"JOB_NAME\" FROM " + JobSchedulerManagedObject.getTableManagedJobs() + " WHERE \"MODEL\"=").append(Integer.toString(workflowModel)).append(" AND \"INPUT_LEVEL\"=").append(Integer.toString(taskLevel));
    ...
    public HashMap getWorkflowModel(String jobName) throws Exception {
        ...
        try {
            query = new StringBuffer("SELECT m.\"ID\", j.\"TITLE\", m.\"NAME\" FROM ").append(JobSchedulerManagedObject.getTableManagedJobs() + " j, ").append(JobSchedulerManagedObject.getTableManagedModels()
                    + " m").append(" WHERE j.\"MODEL\"=m.\"ID\" AND j.\"SUSPENDED\"=0").append(" AND j.\"JOB_NAME\"='" + jobName.replaceAll("'", "''") + "'").append(" AND m.\"SUSPENDED\"=0").append(" AND (m.\"SPOOLER_ID\"='"
                    + this.spooler.id().toLowerCase() + "' OR m.\"SPOOLER_ID\" IS NULL)");
    ...
    public ArrayList getWorkflowModels(String id) throws Exception {
        ...
        try {
            query = new StringBuffer("SELECT \"ID\", \"TITLE\", \"NAME\" FROM ").append(JobSchedulerManagedObject.getTableManagedModels()).append(" WHERE \"SUSPENDED\"=0").append(" AND (\"SPOOLER_ID\"='"
                    + this.spooler.id().toLowerCase() + "' OR \"SPOOLER_ID\" IS NULL) " + " AND " + whereId);
    ...
    private ArrayList getJobsOfWorkflowModel(int modelId, String jobId) throws Exception {
        ...
        try {
            if (modelId > 0) {
                query = new StringBuffer("SELECT j.\"ID\", j.\"TITLE\", j.\"JOB_NAME\", j.\"MODEL\", j.\"INPUT_LEVEL\", j.\"SPOOLER_ID\",").append(" j.\"OUTPUT_LEVEL\", j.\"ERROR_LEVEL\", j.\"JOB_LOCK\", j.\"TIMEOUT\", j.\"PRIORITY\","
                        + " j.\"TASKS\", j.\"IDLE_TIMEOUT\", j.\"MIN_TASKS\", j.\"FORCE_IDLE_TIMEOUT\" FROM ").append(JobSchedulerManagedObject.getTableManagedJobs()
                        + " j, " + JobSchedulerManagedObject.getTableManagedModels() + " m").append(" WHERE j.\"MODEL\"=m.\"ID\" AND m.\"SUSPENDED\"=0").append(whereJobId).append(" AND (m.\"SPOOLER_ID\"='"
                        + this.spooler.id().toLowerCase() + "' OR m.\"SPOOLER_ID\" IS NULL)").append(" AND (j.\"SPOOLER_ID\"='"
                        + this.spooler.id().toLowerCase() + "' OR j.\"SPOOLER_ID\" IS NULL)").append(" AND j.\"SUSPENDED\"=0 AND m.\"ID\"=").append(modelId).append(" ORDER BY j.\"OUTPUT_LEVEL\" ASC");
            } else {
                query = new StringBuffer("SELECT j.\"ID\", j.\"TITLE\", j.\"JOB_NAME\", j.\"MODEL\", j.\"INPUT_LEVEL\", j.\"SPOOLER_ID\",").append(" j.\"OUTPUT_LEVEL\", j.\"ERROR_LEVEL\", j.\"JOB_LOCK\", j.\"TIMEOUT\", j.\"PRIORITY\","
                        + " j.\"TASKS\", j.\"IDLE_TIMEOUT\", j.\"MIN_TASKS\", j.\"FORCE_IDLE_TIMEOUT\" FROM ").append(JobSchedulerManagedObject.getTableManagedJobs()
                        + " j").append(" WHERE j.\"MODEL\"=0").append(whereJobId).append(" AND (j.\"SPOOLER_ID\"='" + this.spooler.id().toLowerCase()
                        + "' OR j.\"SPOOLER_ID\" IS NULL)").append(" AND j.\"SUSPENDED\"=0").append(" ORDER BY j.\"OUTPUT_LEVEL\" ASC");
                /*
                 ...
                 * .append(JobSchedulerManagedObject.getTableManagedJobs() +
                 * " j, " + JobSchedulerManagedObject.getTableManagedModels() +
                 * " m") .append(
                 * " WHERE j.\"MODEL\"=m.\"ID\"(+) AND m.\"SUSPENDED\"(+)=0 AND j.\"SUSPENDED\"=0"
                 * ) .append(" AND (m.\"SPOOLER_ID\"(+)='" +
                 * this.spooler.id().toLowerCase() + "')") .append(whereJobId)
                 * .append(" AND (j.\"SPOOLER_ID\"='" +
                 * this.spooler.id().toLowerCase() +
                 * "' OR j.\"SPOOLER_ID\" IS NULL)")
                 * .append(" ORDER BY j.\"OUTPUT_LEVEL\" ASC"); } else { query =
                 * new StringBuffer(
                 * "SELECT j.\"ID\", j.\"TITLE\", j.\"JOB_NAME\", j.\"MODEL\", j.\"INPUT_LEVEL\", j.\"SPOOLER_ID\","
                 * ) .append(
                 * " j.\"OUTPUT_LEVEL\", j.\"ERROR_LEVEL\", j.\"JOB_LOCK\", j.\"TIMEOUT\", j.\"PRIORITY\","
                 ...
                 * .append(" ON j.\"MODEL\"=m.\"ID\"") .append(
                 * " WHERE (m.\"SUSPENDED\"=0 OR m.\"SUSPENDED\" IS NULL) AND j.\"SUSPENDED\"=0"
                 * ) .append(whereJobId) .append(" AND (m.\"SPOOLER_ID\"='" +
                 * this.spooler.id().toLowerCase() +
