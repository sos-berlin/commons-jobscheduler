...
public class JobSchedulerManagedUserJob extends JobSchedulerManagedJob {
    ...
    public boolean spooler_init() {
        ...
        try {
            ...
            ArrayList hostPort = getConnection().getArray("SELECT \"NAME\", \"WERT\" FROM " + JobSchedulerManagedObject.getTableManagedUserVariables()
            ...
            // spooler.set_var("scheduler_managed_user_job.host",
            // spooler.hostname());
            // config.close();
        } catch (Exception e) {
            try {
            ...
            if (deleted) {
                ...
                if (this.getMaxOrderCount() > 0) {
                    if (spooler.job_chain(orderAttributes.get("job_chain").toString()).order_count() >= this.getMaxOrderCount()) {
                        this.getLogger().info(".. current order [" + orderAttributes.get("id").toString() + "] skipped: order queue length ["
                 ...
                 * this.getLogger().warn("no order in queue"); return
                 * flgOperationWasSuccessful; }
                 */

                try { // to add the order
                      // String answer =
                      // spooler.execute_xml("<remove_order job_chain=\""+orderAttributes.get("job_chain").toString()+
                      // "\" order=\""+order.id()+"\" />");
                    // if(answer.toLowerCase().indexOf("error")>0) throw new
                    // Exception(answer);
                ...
                this.getLogger().info("order [" + orderAttributes.get("id").toString() + "] added to job chain [" + orderAttributes.get("job_chain").toString()
                        + "]: " + order.title());
            }
    ...
    public void spooler_exit() {
         ...
         * ()+ " WHERE \"NAME\"='scheduler_managed_user_job.port'");
         * getConnection
         * ().execute("DELETE FROM "+spooler.db_variables_table_name()+
         * " WHERE \"NAME\"='scheduler_managed_user_job.host'");
         * getConnection().commit(); } catch (Exception e) {}
         */
        super.spooler_exit();
    }
    ...
    private boolean isOver(String runTime) {
        try {
            ...
            if (!runtimeElement.getNodeName().equalsIgnoreCase("run_time"))
                return false;
            node = runtimeElement.getFirstChild();
            while (node != null && node.getNodeType() != Node.ELEMENT_NODE)
                node = node.getNextSibling();
            Element dateElement = (Element) node;
            if (!dateElement.getNodeName().equalsIgnoreCase("date"))
                return false;
