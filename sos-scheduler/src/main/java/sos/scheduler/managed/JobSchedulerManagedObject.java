...
public class JobSchedulerManagedObject {
    ...
    /** Gibt ein Connection Object zurück, welches die Order-spezifische
     * Verbindung darstellt. Funktioniert mit Managed Job 2
     * 
     * @param conn Connection, die benutzt wird, um die Daten für die neue
     *            Connection zu finden
     * @param job job Object, von dessen Order die Connection besorgt werden
     *            soll (meistens "this")
     * @return Connection, die zum Order gehört. Diese ist noch nicht
     *         initialisiert, es müssen also noch connect() und später
     *         disconnect() aufgerufen werden.
     * @throws Exception */
    public static SOSConnection getOrderConnection(final SOSConnection connection, final Job_impl job) throws Exception {
        ...
        if (job.spooler_task.job().order_queue() != null && useManagedConnection) {
            ...
            if (managedVersion.equalsIgnoreCase("1")) {
                 ...
                 * if (connectionName == null || connectionName.length() == 0) {
                 * jobChainModel =
                 * connection.getSingleValue("SELECT \"ID\" FROM " +
                 * getTableManagedModels() + " WHERE \"NAME\"='" + jobChain +
                 * "'"); if (jobChainModel == null || jobChainModel.length() ==
                 * 0) throw (new Exception("no model found for job chain [" +
                 * jobChain + "]of managed order: " + orderId)); }
                 */
            }
    ...
    /** Gibt ein Connection Object zurück, welches die Job-spezifische Verbindung
     * darstellt
     * 
     * @param connection Connection, die benutzt wird, um die Daten für die neue
     *            Connection zu finden
     * @param spoolerId ID des Schedulers
     * @param jobName Name des Jobs
     * @param log SOSLogger für Logging der Datenbankverbindung
     * @return Connection, die zum Job gehört. Diese ist noch nicht
     *         initialisiert, es müssen also noch connect() und später
     *         disconnect() aufgerufen werden.
     * @throws Exception */
    public static SOSConnection getJobConnection(final SOSConnection connection, final String spoolerId, final String jobName, SOSLogger log) throws Exception {
        ...
        try {
            if (log != null) {
                ...
                localConnection = SOSConnection.createInstance(result.get("class").toString(), result.get("driver").toString(), result.get("url").toString(), result.get("username").toString(), result.get("password").toString(), log);
            } else {
                localConnection = SOSConnection.createInstance(result.get("class").toString(), result.get("driver").toString(), result.get("url").toString(), result.get("username").toString(), result.get("password").toString());
            }
     ...
     * @param order Id des Auftrags (order.id())
     * @param databaseConnection Identifizierer der Datenbankverbindung aus
     * @param log SOSLogger für Logging der Datenbankverbindung (aus
     *            order.payload.value("database_connection") in javascript)
     * @return Connection, die zum Job gehört. Diese ist noch nicht
     *         initialisiert, es müssen also noch connect() und später
     *         disconnect() aufgerufen werden.
     * @throws Exception */
     ...
     * 
     * @param connection Connection, which will be used to find the data for the
     *            command
     ...
     * @param connection Connection, which will be used to find the data for the
     *            command
     * @param job job object whose order's command shall be returned (e.g.
     *            "this") *
     * 
     * @throws Exception */
    ...
    /** If a managed order is created inside the Job Scheduler it is assigned a
     * new ID. This method returns the original ID from the
     * SCHEDULER_MANAGED_ORDERS table.
     * 
    ...
    /** replaces variables of the ${name} and §{name} schemes in a command with
     * values from Variable_set
     * 
