...
public class JobSchedulerLogAnalyser extends JobSchedulerJob {
			...
			try {
				...
				while((logLine = checkFile.readLine()) != null) {
				   ...
				   if (lastLog_date.after(actlog_date)  ) {
					...
					if (logLine.matches( getRegularExpression("WARN"))  ) {
					    ...
					    if (logLine.matches( getRegularExpression("ERROR") )) {
				...
				while(itKeys.hasNext()) {
					...
					for (int i = 0; i < stateList.size(); i++) {		
						...
						if(actMessage.severity.equalsIgnoreCase("WARN") || msgLine.matches( getRegularExpression("WARN"))){
						...
						} else if(actMessage.severity.equalsIgnoreCase("ERROR") || msgLine.matches( getRegularExpression("ERROR"))){
	...
	private void getParameters() throws Exception {
		...
		try { // to fetch parameters from the job and from orders that have precedence to job parameters
           ...
           if (sosString.parseToString(spooler_task.params().var("log_line_regular_expression")).length() > 0) {
