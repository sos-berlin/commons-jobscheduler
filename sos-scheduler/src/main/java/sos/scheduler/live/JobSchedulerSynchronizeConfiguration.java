...
public class JobSchedulerSynchronizeConfiguration extends JobSchedulerJob {
        ...
        if (element.table_name.equals("")) {
            ...
            if (resultset.hasNext()) {
                ...
                while (fieldnames.hasNext()) {
                    ...
                    if (!f.equals("object_path") && !f.equals("cdata") && !f.equals("parent_id") && !f.equals("pk_id") && !f.equals("object_id")
                            && rec.get(f) != null && !rec.get(f).equals("")) {
                        attr += f;
                        attr += "=\"" + rec.get(f) + "\"" + " ";
    ...
    /*
     * private String getBooleanValue(HashMap h, String k) { String erg = "";
     * String s = getValue(h,k); if (s.equals("1")) { erg = "yes"; }else { erg =
     * "no"; } return erg; } private int getIntValue(String s, int d) { try {
     * int i = Integer.parseInt(s); return i; }catch (NumberFormatException n) {
     * return d; } }
     */
