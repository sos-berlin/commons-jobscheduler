...
/** Klasse JobSchedulerLoadTestLauncherJob Diese Job kann die Ausführung
 * paralleler Jobs skalieren und beliebig viele Jobs können parallel laufen. Der
 * Job kann beliebig viele individuelle Jobs startet und kann mit dem Namen
 * eines anderen Jobs oder Auftrags parametrisiert werden, die gestartet werden
 * soll. Die Anzahl zu startender Jobs in konfigurierbaren Zeitabständen sowie
 ...
 * 
 *          resourcen: * sos.mail.jar, sos.util.jar, sos.xml.jar,
 ...
 *
 *          Der Host des Job Schedulers, der den Job ausführt, Default:
 *          localhost
 *
 ...
 *
 *          Die minimale Anzahl von Jobs oder Aufträgen, die gleichzeitig
 *          gestartet wird. Default=1
 *
 *          d) <param name="scheduler_launcher_max_starts" value="100"/>
 *
 *          Die maximale Anzahl von Jobs oder Aufträgen, die gleichzeitig
 *          gestartet werden.
 *
 *          e) <param name="scheduler_launcher_start_increment" value="+10|*2"/>
 *
 *          Die Anzahl zu startender Job oder Aufträge wird pro Start um diese
 *          Zahl erhöht. Ein Wert 3 ist gleichbedeutend mit +3, d.h. die Zahl
 *          wird um 3 erhöht. Ein Wert *2 bedeutet, dass sich die Anzahl
 *          verdoppelt. Das Inkrement gilt nicht beim ersten Start. Die mit dem
 *          Parameter max_starts gesetzte Anzahl darf nicht überschritten
 *          werden. Wird sie überschritten, dann werden alle weiteren Starts mit
 *          dem Wert von max_starts ausgeführt.
 *
 *          f) alle Parameter, deren Namen nicht mit scheduler_launcher_
 *          beginnen, werden an den Job oder Auftrag durchgereicht.
 *
 *          2.2 Spezielle Parameter für Jobs, siehe Beispiel <param
 *          name="scheduler_launcher_job" value="job_name"/> Wenn dieser
 *          Parameter übergeben wurde, dann handelt es sich um einen Job. Der
 *          hier übergebene Job-Name ist der Wert des Attributs job= in
 *          <start_job job="..."/>
 *
 ...
 *          a) mandatory keine
 *
 *          b) optional, d.h. bitte keine eigenen Defaults, sondern im Fall des
 *          Fehlens einfach nicht übergeben <param
 *          name="scheduler_launcher_job_after" value="..."/> <param
 *          name="scheduler_launcher_job_at" value="..."/> <param
 *          name="scheduler_launcher_job_web_service" value="..."/>
 *
 *          2.3 Spezielle Parameter für Aufträge, siehe Beispiel <param
 *          name="scheduler_launcher_order" value="order_id"/> Wenn dieser
 *          Parameter übergeben wurde, dann handelt es sich um einen Auftrag.
 *          Die hier übergebene Auftragskennung ist der Wert des Attributs id=
 *          in <add_order id="..."/>
 *
 *          In diesem Fall werden ausgewertet:
 *
 *          a) mandatory <param name="scheduler_launcher_order_job_chain"
 *          value="..."/>
 *
 *          b) optional, d.h. bitte keine eigenen Defaults, sondern im Fall des
 *          Fehlens einfach nicht übergeben <param
 *          name="scheduler_launcher_order_replace" value="yes|no"/> <param
 *          name="scheduler_launcher_order_state" value="..."/> <param
 *          name="scheduler_launcher_order_title" value="..."/> <param
 ...
 * 
 *
 *          2.4 Alle anderen Parameter werden durchgereicht. Genauer: alle
 *          Parameter, die nicht mit scheduler_launcher_ beginnen, werden den
 *          Jobs übergeben, die gestartet werden sollen. */
