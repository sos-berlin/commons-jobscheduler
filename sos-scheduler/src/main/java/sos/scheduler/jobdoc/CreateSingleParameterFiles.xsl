...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="//jobdoc:params">
		<xsl:for-each select="./jobdoc:param">
			<xsl:if test="./@name != '' and ./@name != '*'">
				<xsl:variable name="DocumentName">
					<xsl:value-of select="concat('file:///c:/temp/params/param_', ./@name, '.xml')"></xsl:value-of>
				</xsl:variable>
				<xsl:message>
					<xsl:value-of select="concat('DocFileName is ', $DocumentName)"></xsl:value-of>
				</xsl:message>
				<xsl:result-document href="{$DocumentName}" 					method="xml">
					<xsl:copy-of select="." />
				</xsl:result-document>
			</xsl:if>
		</xsl:for-each>
		...
		<xsl:variable name="IncludesDocumentName">
			<xsl:value-of select="concat('file:///c:/temp/params/param_', 'includes', '.xml')"></xsl:value-of>
		</xsl:variable>
				...
				<xsl:if test="./@name != '' and ./@name != '*'">
					<xsl:element name="xi:include" namespace="http://www.w3.org/2001/XInclude">
						<xsl:attribute name="href"
							select="concat('./params/param_', ./@name, '.xml')" />
