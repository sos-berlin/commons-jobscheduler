...
public class CronConverter extends JSToolBox {
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            Option optTimeOut = OptionBuilder.create(OPTION_TIMEOUT);
    ...
    public CronConverter(final SOSLogger log) throws Exception {
        ...
        cronRegExPuppetNamePattern = Pattern.compile(CRON_REGEX_PUPPETNAME);
        cronRegExJobTitlePattern = Pattern.compile(CRON_REGEX_JOBTITLE);
        cronRegExJobTimeoutPattern = Pattern.compile(CRON_REGEX_JOBTIMEOUT);
    ...
    public Document createJobElement(String cronline, final HashMap<String, String> environmentVariables) throws Exception {
        try {
            ...
            while (envIter.hasNext()) {
                ...
                script += "export " + envName + NEWLINE;
            }
            script += "echo created by " + CLASSNAME + ", at " + SOSDate.getCurrentTimeAsString() + NEWLINE;
            if (isCreateAMock()) {
        ...
        try {
            ...
            if (minutes.startsWith("*")) {
                ...
                if (hours.startsWith("*")) {
                    if (!"*".equalsIgnoreCase(hours)) {
                        // repeat interval is given for hours and minutes.
                        // Doesn't make sense.
                ...
                for (String element : minutesArray) {
                    ...
                    if (hours.startsWith("*")) {
                        ...
                        usedNewRunTime = true;
                        if (!"*".equalsIgnoreCase(hours)) {
                            // repeat interval is given for hours
            ...
            if (days.startsWith("*")) {
                if (!"*".equals(days)) {
                    ...
                    // this is not possible in the JobScheduler but can be
                    // poorly emulated
