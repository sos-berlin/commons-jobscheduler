...
public class JobSchedulerExportJob extends JobSchedulerJob { 
	...
	public boolean spooler_init() {
		...
		if(rc){
			try {
				...
				if (this.getParameters().value("export_path") != null && 
						this.getParameters().value("export_path").length() > 0) {
				...
				//DB Einstellungen aus der sos_settings.file (=jobparameter) oder aus der factory.ini
	...
	public boolean spooler_process() {
		...
		try {
			...
			if (this.getParameters().value("operation") != null && 
					this.getParameters().value("operation").length() > 0) {
			...
			//System.out.println("successfully export Database to XML-File " + filename);
			...
			if(sosConnection != null) {
				try {
					//System.out.println("Fehler: " + SOSClassUtil.getMethodName() + e.toString());
					sosConnection.rollback();
				} catch (Exception ex) {}
				try {					
					String upStr = "UPDATE "  +
					JobSchedulerSignalJob.TABLE_SCHEDULER_SIGNAL_OBECTS + 
					" SET \"STATUS\" = 1002 WHERE  \"SIGNAL_ID\" = " + signalId;
					sosUpdateStateConnection.executeUpdate(upStr);
					sosUpdateStateConnection.commit();
	 ...
	 * 
	 *  Die neue Dateiname wird gebildet aus: 
...
//			get the root element
			...
			Document doc = builder.build( new File( queryfile ) );
			
			Element root = doc.getRootElement();
			
			if(root != null) {
				listOfQueries = root.getChildren("queries");
				for(int i = 0; i < listOfQueries.size(); i++) {
					Element elem = (Element)listOfQueries.get(i);
			...
			if (queries != null) {
				...
				for (int i =0; i < listOfQueries.getLength(); i++) {
					...
					if(child.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
						...
						//REKURSIVE AUFRUF WENN DAS ELEMENT QUERIES BZW. QUERY WEITERE QUERY KINDKNOTEN HAT
						newlistOfQueries = elem.getElementsByTagName("query");
						//int exportid ermitteln und den rekursiven Teil wiedergeben 
