...
public class JobSchedulerCleanupHistory extends Job_impl {
    ...
    /** Attribut: name of the order history table (for Uwe's old Solaris
     * Scheduler) */
    ...
    public boolean spooler_init() {
        ...
        try {
            ...
            if (spooler_task.params().var("delete_jade_history_interval") != null
                    && spooler_task.params().var("delete_jade_history_interval").length() > 0) {
            ...
            if (spooler_task.params().var("delete_daily_plan_interval") != null
                    && spooler_task.params().var("delete_daily_plan_interval").length() > 0) {
            ...
            if (spooler_task.params().var("cleanup_order_history_table") != null
                    && spooler_task.params().var("cleanup_order_history_table").length() > 0) {
            ...
            if (spooler_task.params().var("cleanup_order_step_history_table") != null
                ...
                cleanupOrderStepHistoryTable = "SCHEDULER_ORDER_STEP_HISTORY"; // wird
                                                                               // leider
                                                                               // nicht
                                                                               // unterstütztz:
            ...
            if (spooler_task.params().var("cleanup_history_log_compressed") != null
                    && spooler_task.params().var("cleanup_history_log_compressed").length() > 0) {
    ...
    public boolean spooler_open() {
        ...
        try {
            ...
            if (cleanupHistoryOrderId != null && cleanupHistoryOrderId.length() > 0) {
                // orderHistoryQuery = " SELECT \"HISTORY_ID\" AS \"ID\" FROM "
                // + cleanupOrderHistoryTable + " WHERE \"ORDER_ID\"='" +
                ...
                // orderHistoryQuery = " SELECT \"HISTORY_ID\" AS \"ID\" FROM "
                // + cleanupOrderHistoryTable + " " + sqlCondition +
                // " ORDER BY \"ID\" ASC";
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            if (cleanupHistoryItemIndex < cleanupHistoryItemMaxHistory) {
                ...
                cleanupOrderHistory(Integer.parseInt(cleanupHistoryOrderItems.get(cleanupHistoryItemIndex++ - cleanupHistoryItemMaxHistory).toString()), cleanupHistoryLogLevel);
            }
        } catch (Exception e) {
    ...
    public void cleanupHistory(int id, int logLevel) throws Exception {
        ...
        try {
            ...
            cleanupHistoryDb.open("-in nl | " + logCompressed + spooler.db_name() + " -table=" + cleanupHistoryTable + " -blob=LOG WHERE \"ID\"="
                    + id);
            ...
            if (!cleanupHistoryDb.eof()) {
                try {
                    ...
                    throw new Exception("error occurred processing log in cleanup for history entry [" + id + "], line " + linesProcessed + ": "
                            + e.toString());
                }
            }
    ...
    public void cleanupOrderHistory(int id, int logLevel) throws Exception {
        ...
        try {
            sos.hostware.File cleanupHistoryDb = new sos.hostware.File();
            cleanupHistoryDb.open("-in nl | " + logCompressed + spooler.db_name() + " -table=" + cleanupOrderHistoryTable
