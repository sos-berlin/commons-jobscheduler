 ...
 * section_mail_templates mail_to log_directory log_only queue_directory
 * max_delivery
 * 
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            if (spooler_task.job().order_queue() == null) {
                ...
                listFile = new File(this.getQueueDirectory() + (!this.getQueueDirectory().endsWith("/") && !this.getQueueDirectory().endsWith("\\") ? "/" : "")
                        + parameters.var("file"));
            }
            ...
            // job scheduler sets mail host if no default was specified by
            // settings
            ...
            // job scheduler sets mail queue directory if no default was
            // specified by parameters or settings
            ...
            try { // to send
                ...
                if (this.getLogger() != null) {
                    if (this.isLogOnly()) {
                        this.getLogger().info("mail was processed from file [" + workFile.getAbsolutePath() + "] to: " + sosMail.getRecipientsAsString()
                ...
                if (this.getConnection() != null && this.hasDatabase() && mailOrderId > 0) {
                    this.getConnection().executeUpdate("UPDATE " + this.getTableMails() + " SET \"JOB_ID\"=" + spooler_task.id() + ", \"STATUS\"="
                            ...
                            + ", \"MODIFIED\"=%now, \"MODIFIED_BY\"='" + spooler_task.job().name() + "', \"DELIVERED\"=%now" + " WHERE \"ID\"=" + mailOrderId);
                ...
                throw new Exception("mail was NOT sent from file [" + workFile.getAbsolutePath() + "]"
                        + (mailOrderId > 0 ? " for order [" + mailOrderId + "]" : "") + ": " + ex.getMessage());
            }
            ...
            // do not create warnings (that are sent by mail) if the number of
            // dequeue trials has exceeded
            ...
            try { // to update database if mail order has been identified
                if (this.getConnection() != null && this.hasDatabase() && mailOrderId > 0) {
                    ...
                    message = message.replaceAll("'", "''");
                    this.getConnection().executeUpdate("UPDATE " + this.getTableMails() + " SET \"JOB_ID\"=" + spooler_task.id() + ", \"STATUS\"="
                            + STATE_ERROR + ", \"STATUS_TEXT\"='" + message + "'," + "\"MODIFIED\"=%now, \"MODIFIED_BY\"='" + spooler_task.job().name()
            ...
            try {
                // this file should only exist in case of errors, we rename it
                // with a prefix to prevent further processing
                if (messageFile.exists()) {
