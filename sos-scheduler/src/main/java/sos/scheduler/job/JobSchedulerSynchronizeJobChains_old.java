 ...
 * @version 2009-09-12 This job can be used - with the same job name - in
 * multiple job chains and would require each chain to have the required number
 * of active orders (default to 1) enqueued. Thus all orders from all chains are
 * set back if for one of these chains the number of required orders has not yet
 * been reached. As soon as all required orders are present then the current
 * order is passed through this chain and all other chains are signalled to make
 * pass through the respective number of required orders.
 */
...
public class JobSchedulerSynchronizeJobChains_old extends Job_impl {
    ...
    // delay order after setback: default interval if not specified by job
    // configuration
    private int setbackInterval = 600;
    // delay order after setback: max. number of setbacks or unbounded if not
    // specified by job configuration
    ...
    public boolean spooler_init() {
        try {
            ...
            // by precedence we use the configuration specified by
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            try { // to get the status information on current job chains
                ...
                // spooler_log.debug6("Job Scheduler status response: " +
                // currentStatusAnswer);
            ...
            try { // to verify the information on the current order and current
                  // job chain
            ...
            try { // to pass the current order if previously signalled by a
                  // global Job Scheduler variable
            ...
            try { // to retrieve the list of dependent job chains nodes
                ...
                // nodes =
                // currentStatusXPath.selectNodeList("/spooler/answer/state/job_chains/job_chain[( job_chain_node/@state = job_chain_node[@job = '"
                // + currentJobPath +
                // "']/@next_state or job_chain_node/@state = job_chain_node[@job = '"
                // + currentJobPath +
                ...
                // if there is only one job chain with one synchronization job
                // node then multiple orders for the same chain get synchronized
                if (nodes != null && nodes.getLength() == 1) {
                    ...
                    // suspend/setback the current order for this job chain: if
                    // this is the last node of a job chain then the order is
                    // being set back, otherwise it will be suspended
            ...
            try { // to check in dependent job chain nodes if the required
                  // number of orders has been enqueued
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    // spooler_log.debug9("node check: self::node()[( job_chain_node/@state = job_chain_node[@job = '"
                    // + currentJobPath +
                    // "']/@next_state or job_chain_node/@state = job_chain_node[@job = '"
                    // + currentJobPath +
                    ...
                    // NodeList orderNodes =
                    // currentStatusXPath.selectNodeList(nodes.item(i),
                    // "self::node()[( job_chain_node/@state = job_chain_node[@job = '"
                    // + currentJobPath +
                    // "']/@next_state or job_chain_node/@state = job_chain_node[@job = '"
                    // + currentJobPath +
                    // "']/@state) and ( (job_chain_node/order_queue/order/@setback_count > 0 and job_chain_node/order_queue/order/@setback != '') or job_chain_node/order_queue/order/@suspended = 'yes')]/job_chain_node/order_queue[@length > 0]/order");
                    ...
                    // Integer.parseInt(nodes.item(i).getAttributes().getNamedItem("orders").getNodeValue());
                    ...
                    // add the currently processed order to the number of
                    // pending orders
                    ...
                    // should any of the dependent job chains have less orders
                    // enqueued than required, then the current order will be
                    // set on hold
                ...
                if (!passOrders) {
                    // suspend/setback the current order for this job chain: it
                    // will be suspended
                    if (setbackType.equalsIgnoreCase("setback")) {
                        spooler_task.order().setback();
                    } else {
                        if (!spooler_task.order().suspended()) {
                            spooler_task.order().set_state(spooler_task.order().state()); // Damit
                                                                                          // der
                                                                                          // Suspend
                                                                                          // auf
                                                                                          // den
            ...
            // signal to other job chains that the next order should be passed
            ...
            if (passOrders) {
                ...
                while (jobChainNodesIterator.hasNext()) {
                    ...
                    if (setbackType.equalsIgnoreCase("setback")) {
                        ...
                        // signal current job chain to pass through the required
                        // number of orders minus the current one
                        if (dependentJobChainPath.equals(currentJobChainPath) && dependentNodeName.equals(currentNodeName)) {
                            ...
                            // make other job chain pass through the required
                            // number of orders
                    ...
                    try { // to signal waiting orders to start immediately
                        ...
                        /*
                         * if (spooler_task.params().value(dependentJobChainName
                         * + "_required_orders") != null &&
                         * spooler_task.params().value(dependentJobChainName +
                         * "_required_orders").length() > 0) { requiredOrders =
                         * Integer.parseInt(spooler_task.params().value(
                         * dependentJobChainName + "_required_orders")); } else
                         * if (spooler_task.params().value("required_orders") !=
                         * null &&
                         * spooler_task.params().value("required_orders")
                         * .length() > 0) { requiredOrders =
                         * Integer.parseInt(spooler_task
                         * .params().value("required_orders")); }
                         */
                        ...
                        for (int i = 0; i < signalNodes.getLength(); i++) {
                            ...
                            if (signalNodes.item(i).getAttributes().getNamedItem("setback") != null) {
                                if (!setbackType.equalsIgnoreCase("setback")) {
                                    String signalVariable = keyName + "_pass_through_next_order";
                                    // signal current job chain to pass through
                                    // the required number of orders minus the
                                    // current one
                                    if (dependentJobChainPath.equals(currentJobChainPath) && dependentNodeName.equals(currentNodeName)) {
                                        if (orders[1] - 1 > 0) {
                                            spooler.set_var(signalVariable, Integer.toString(orders[1] - 1));
                                        ...
                                        // make other job chains pass through
