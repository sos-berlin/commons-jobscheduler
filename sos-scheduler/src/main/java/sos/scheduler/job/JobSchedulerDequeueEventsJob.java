...
//                }
                ...
                if (isNotEmpty(this.getParameters().value("supervisor_port"))) {
                    try {
                        ...
                        throw new JobSchedulerException("illegal non-numeric value for Supervisor Job Scheduler port specified: " + this.getParameters().value("supervisor_port"), ex);
                    }
                } else  {
                	if (supervisor != null && isNotEmpty(supervisor.hostname())) {
                ...
                if (isNotEmpty(this.getParameters().value("supervisor_timeout"))) {
                    try {
                        ...
                        throw new JobSchedulerException("illegal non-numeric value for Supervisor Job Scheduler timeout specified: " + this.getParameters().value("supervisor_timeout"), ex);
                    }
                } else {
...
//                }
                ...
                while ((strValues = hwFile.readCSVLine()) != null) {
                    ...
                    if (strValues.length > eventMinFieldCount) {
//                    	this.getLogger().info("--->" + strValues[10] + " " + strValues[10].indexOf(""));
...
//                        if (record.string(10).indexOf("=") == -1) {// Kompatibilitätsabfrage
...
//                }
                ...
                throw new JobSchedulerException("error occurred forwarding events to Supervisor Job Scheduler [" + this.getEventSupervisorSchedulerHost() + ":" + this.getEventSupervisorSchedulerPort() + "]: " + e.getMessage(), e);
            }
