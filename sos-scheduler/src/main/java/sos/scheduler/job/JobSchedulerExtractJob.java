 ...
 *
 *         This job is used to extract records from a csv file or from a
 *         database to a file */

public class JobSchedulerExtractJob extends JobSchedulerJob {

    /** file name samples: - default without field names: -in
     * -type=(ppid,pid,client
     * ,application,process,host,account,script,log_file,start_time
     * :Datetime('yyyy-mm-dd HH:MM:SS'),end_time:Datetime('yyyy-mm-dd
     * HH:MM:SS'),state,message) tab -csv - sample: -in tab -csv -field-names ->
     * separator=; delimiter=" - sample: -in tab -quote='x' -tab='y'
     * -field-names -> separator=y delimiter=x - sample for ODBC: -in odbc
     * -conn-str='DRIVER=Microsoft Access Driver
     * (*.mdb);DBQ=c:\loginfo\application_loginfo.mdb' SELECT "PPID", "PID",
     * "CLIENT", "APPLICATION", "PROCESS", "HOST", "ACCOUNT", "SCRIPT_FILE" as
     * "SCRIPT", "LOG_FILE", "START_TIME", "END_TIME", "STATE", "LOG_MESSAGE" as
     * "MESSAGE" FROM APPLICATION_LOGS ORDER BY "START_TIME" ASC - sample for
     * JDBC: -in jdbc -class=oracle.jdbc.driver.OracleDriver
     * jdbc:oracle:thin:@localhost:1521:orcl -user=appman -password=appman
     * SELECT "PPID", "PID", "CLIENT", "APPLICATION", "PROCESS", "HOST",
     * "ACCOUNT", "SCRIPT_FILE" as "SCRIPT", "START_TIME", "END_TIME", "STATE",
     * "LOG_FILE", "LOG_MESSAGE" as "MESSAGE" FROM APPLICATION_LOGS ORDER BY
     * "START_TIME" ASC */
    ...
    /** character that quotes the field delimiter within field values of output
     * files */
        ...
        try {
            ...
            try {
                ...
                if (this.getParameters().value("output_field_delimiter_quote") != null
                        && this.getParameters().value("output_field_delimiter_quote").length() > 0) {
                ...
                if (this.getInputFilenamePrefix().toLowerCase().indexOf("-class=") != -1
                        ...
                        || this.getInputFilePath().toLowerCase().indexOf("-class=") > -1 || this.getInputFilePath().toLowerCase().indexOf("-conn-str=") > -1) {
                ...
                if (this.getOutputFilenamePrefix().toLowerCase().indexOf("-class=") != -1
                        ...
                        || this.getOutputFilePath().toLowerCase().indexOf("-class=") > -1 || this.getOutputFilePath().toLowerCase().indexOf("-conn-str=") > -1) {
            ...
            if (this.getInputFileType().equals("database")) {
                ...
                if (outputFile.isDirectory())
                    throw new Exception("for an input database query specified by the parameter [input_file] an output file instead of a directory must be specified by the parameter [output_file]");
                ...
                if (inputFile.isDirectory()) {
                    if (!outputFile.isDirectory())
                        throw new Exception("for an input directory specified by the parameter [input_file] an output directory must be specified by the parameter [output_file]");
                    ...
                    while (iterator.hasNext()) {
                        ...
                        outputFile = new File(this.getOutputFilePath() + "/" + extractFile.getName().substring(0, extractFile.getName().lastIndexOf("."))
                    ...
                    if (outputFile.isDirectory())
                        throw new Exception("for an input file specified by the parameter [input_file] an output file must be specified by the parameter [output_file]");
            ...
            spooler_log.error("error occurred extracting records from " + (this.getInputFileType().equals("file") ? this.getInputFilePath() : "database query")
                    + ": " + e.getMessage());
            return false;
        }
    }
    ...
    /*
     * extract from database query or csv file to target file or database and
     * return number of successfully extracted records
     */
    public long extract(File inputFile, File outputFile) throws Exception {
        ...
        try {
            if (this.getOutputFieldDelimiterQuote().equals("\\"))
            ...
            if (this.getOutputFieldNames() != null && this.getOutputFieldNames().length() > 0) {
                if (this.getOutputFieldNames().equalsIgnoreCase("true") || this.getOutputFieldNames().equalsIgnoreCase("yes")
                        || this.getOutputFieldNames().equals("1")) {
                    outputFieldNamesWrite = true;
                } else if (!this.getOutputFieldNames().equalsIgnoreCase("false") && !this.getOutputFieldNames().equalsIgnoreCase("no")
                        && !this.getOutputFieldNames().equals("0")) {
            ...
            // ... should no output fields have been found, then lookup output
            // fields from the input file specification
            ...
            while (!inFile.eof()) {
                ...
                // should no output fields have been found, then use them from
                // the first ecord
                            ...
                            if (fieldType != null && fieldType.toLowerCase().equals("string")) {
                                if (this.getOutputFieldDelimiterQuote().length() > 0) {
                                    ...
                                    line += this.getOutputFieldDelimiter()
                                            + record.string(fieldName).replaceAll("\\" + this.getOutputFieldDelimiter(), this.getOutputFieldDelimiterQuote()
                                                    + this.getOutputFieldDelimiter()) + this.getOutputFieldDelimiter();
                                    outputFieldCount++;
                                } else {
                                    if (outputFieldCount > 0)
                                        line += this.getOutputFieldSeparator();
                                ...
                                try {
                                    if (outputFieldCount > 0)
                                        line += this.getOutputFieldSeparator();
                                    // line += this.getOutputFieldDelimiter() +
                                    // SOSDate.getDateAsString(SOSDate.getDate(record.string(fieldName),
                                    // SOSDate.dateTimeFormat),
                                    // this.getOutputDateFormat()) +
                                    // this.getOutputFieldDelimiter();
                                    line += this.getOutputFieldDelimiter()
                                            + SOSDate.getDateAsString(SOSDate.getDate(record.string(fieldName), this.getInputDateFormat()), this.getOutputDateFormat())
                                            + this.getOutputFieldDelimiter();
                                    outputFieldCount++;
                                } catch (Exception e) {
            ...
            this.getLogger().info(recordCount + " records found, " + errorRecordCount + " errors, " + successRecordCount + " records extracted from "
    ...
    public static String getDateReplacement(String fileName) throws Exception {
        ...
        /*
         * group(0): our string itself group(1): prefix group(2): [date:
         * group(3): white space group(4): yyyyMMdd_HHmmss group(5): white space
         * group(6): ] group(7): rest of string
         */
