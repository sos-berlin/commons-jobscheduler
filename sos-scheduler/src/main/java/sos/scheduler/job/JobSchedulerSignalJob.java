	...
	public void getTaskParameters(boolean logValue) throws Exception {
		...
		try { 
			...
			if (spooler_task.params().var("sos_settings_file") != null && spooler_task.params().var("sos_settings_file").length() > 0) {
				...
				if (logValue) spooler_log.info(".. job parameter [sos_settings_file]: " + this.getSosSettingsFile());
			}
...
//			Status: 0=Auftrag ist offen
			String selStr = "SELECT \"SIGNAL_ID\"," +
			"  \"JOB_CHAIN\",  " +
			"  \"OPERATION\"  " +
			"  FROM " + TABLE_SCHEDULER_SIGNAL_OBECTS +
			"  WHERE  \"STATUS\" = 0";
	...
	public void showSummary() throws Exception {
		...
		try {
			...
			String stateText = "..number of create order: " + (countProcessOk + countProcessError) + "(error="+countProcessError +  ";successfully="+countProcessOk +")";
