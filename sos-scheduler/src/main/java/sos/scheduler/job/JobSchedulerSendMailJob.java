...
public class JobSchedulerSendMailJob extends JobSchedulerMailJob {
    ...
    public boolean spooler_open() {
        ...
        try {
            ...
            if (spooler_task.job().order_queue() == null) {
                this.mailOrders = this.getConnection().getArrayValue("SELECT \"ID\" FROM " + this.getTableMails() + " WHERE \"STATUS\"="
                        + STATE_READY + " AND (\"TARGETED\" IS NULL OR \"TARGETED\"<=%now) ORDER BY \"ID\" ASC");
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            // set queue prefix "sos" to enalbe dequeueing by
            ...
            try { // to check status: allow all status values except
                  // STATE_SUCCESS and STATE_CANCEL
                ...
                if (this.getLogger() != null)
                    this.getLogger().info("mail status [" + mailOrder.getStatus() + "] not applicable for order [" + mailOrder.getId() + "]: "
                            + ex.getMessage());
            ...
            try {
                ...
                if (this.getMailTo() != null && this.getMailTo().length() > 0) {
                    if (this.getLogger() != null)
                        this.getLogger().info("mail recipients [" + mailOrder.getRecipientsAsString() + "] are replaced by settings: "
                                + this.getMailTo());
                ...
                if (this.getLogger() != null)
                    this.getLogger().info("mail was " + (this.isLogOnly() ? "processed" : "sent") + " for order " + mailOrderId + " to: "
