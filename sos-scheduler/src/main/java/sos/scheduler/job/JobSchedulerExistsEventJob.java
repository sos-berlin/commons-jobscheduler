...
public class JobSchedulerExistsEventJob extends JobSchedulerJob {
    ...
    public boolean spooler_process() throws JobSchedulerException {
        ...
        try {
            ...
            if (params.var("scheduler_event_service_id") != null && !params.var("scheduler_event_service_id").isEmpty()) {
    ...
    private void readEventsFromDB(final SOSConnection conn, final Spooler spooler, final Document eventsDoc, final SOSLogger log) throws Exception {
        try {
            conn.executeUpdate("DELETE FROM " + tableEvents
                    ...
                    + "')");
            conn.commit();
            Vector<?> vEvents = conn.getArrayAsVector("SELECT \"SPOOLER_ID\", \"REMOTE_SCHEDULER_HOST\", \"REMOTE_SCHEDULER_PORT\", \"JOB_CHAIN\", \"ORDER_ID\", "
                    + "\"JOB_NAME\", \"EVENT_CLASS\", \"EVENT_ID\", \"EXIT_CODE\", \"CREATED\", \"EXPIRES\", \"PARAMETERS\" FROM "
                    + tableEvents + " WHERE (\"SPOOLER_ID\" IS NULL OR \"SPOOLER_ID\"='' OR \"SPOOLER_ID\"='" + scheduler_event_service_id 
                    + "') ORDER BY \"ID\" ASC");
