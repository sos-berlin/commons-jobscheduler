...
public class DbmsOutput {
     ...
     * high. Also -- make sure to use BIND VARIABLES! we use three statments in
     * this class. One to enable dbms_output - equivalent to SET SERVEROUTPUT on
     * in SQL*PLUS. another to disable it -- like SET SERVEROUTPUT OFF. the last
     * is to "dump" or display the results from dbms_output using system.out
     */
     ...
     * executing. the statement we prepare for SHOW is a block of code to return
     * a String of dbms_output output. Normally, you might bind to a PLSQL table
     * type but the jdbc drivers don't support PLSQL table types -- hence we get
     * the output and concatenate it into a string. We will retrieve at least
     * one line of output -- so we may exceed your MAXBYTES parameter below. If
     * you set MAXBYTES to 10 and the first line is 100 bytes long, you will get
     * the 100 bytes. MAXBYTES will stop us from getting yet another line but it
