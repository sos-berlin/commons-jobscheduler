...
public class JobSchedulerWebServiceShellJob extends Job_impl {
    ...
    public boolean spooler_process() {
        ...
        try {
            ...
            /*
             * these are samples for shell commands: if (
             * System.getProperty("os.name").toLowerCase().startsWith("win") ) {
             * // Windows: the command to be executed, modify this to an
             * executable script for your platform commandline =
             * "cmd.exe /V:ON /C " + spooler.directory() +
             * "samples/shell.webservice.command/webservice_shell.cmd"; } else {
             * // Unix: the command to be executed, modify this to an executable
             * script for your platform commandline = spooler.directory() +
             * "samples/shell.webservice.command/webservice_shell.sh"; }
             */

            // retrieve parameters from order payload (<job order="yes"/>) or
            ...
            if (parameters != null) {
                ...
                for (int i = 0; i < parameter_names.length; i++) {
                    if (parameter_names[i].equalsIgnoreCase("shell_command"))
                        continue;
                    // .. either append parameter names and values to the
                    // comamnd line (this needs more checking for
                    // vulnerabilities)
                    // spooler_log.debug3( "parameter " + parameter_names[i] +
                    // "=" + parameters.value(parameter_names[i]) );
                    // commandline += " " + parameter_names[i] + "=\"" +
                    // parameters.value(parameter_names[i]) + "\"";

                    // .. or move parameters to environment variables which is
                    // more safe
                    subprocess.set_environment(parameter_names[i], parameters.value(parameter_names[i]));
                    // .. and for this sample append parameter names to the
                    // command line
                    commandline += " \"" + parameter_names[i] + "\"";
                }
            }
            ...
            // wait until the subprocess terminates, timeout is restricted by
            // <job timeout="..."/> or subprocess.wait_for_termination(60)
