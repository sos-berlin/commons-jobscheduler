...
/** <p>
 * JobSchedulerProcessDatabaseJob implements an order driven job that executes
 * database statements.
 * </p>
 * 
...
public class JobSchedulerProcessDatabaseJob extends ProcessOrderJob {
        ...
        try {
            ...
            try { // to process order
                ...
                try { // to get the database connection
                    ...
                    this.setConnection(SOSConnection.createInstance(this.getParameters().value("db_class"), this.getParameters().value("db_driver"), this.getParameters().value("db_url"), this.getParameters().value("db_user"), this.getParameters().value("db_password"), this.getLogger()));
    ...
    /** <p>
     * Die übergebene SOSConnection ist bereits verbunden, sollte aber nach
     * Ausführung der Statements via commit/rollback abgeschlossen werden. Die
     * Standardimplementierung der Funktion ist:
     * </p>
     * <p>
     * <code>connection.executeStatements(command);
     * </p>
     * 
