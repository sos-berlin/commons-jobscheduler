...
/** <p>
 * ProcessOrderJob ist die Basisklasse für auftragsgesteuerte Jobs und stellt
 * Methoden für Jobs zur Verfügung:
 * </p>
 * <p>
 * - spooler_init(): stdout und stderr als streams lesen
 * </p>
 * <p>
 * - spooler_exit(): cleanup
 * </p>
 * <p>
 * - executeSubprocess(): Subprozesse ausführen
 * </p>
 * <p>
 * - prepare(), prepareParameters(), prepareAttributes(): Parameter des Jobs und
 * des Auftrags zusammenführen
 * </p>
 * <p>
 * - initConfiguration(): XML-Konfigurationsdatei als XML payload an den Auftrag
 * übergeben
 * </p>
 * <p>
 * - prepareConfiguration(): liefert die XML-Konfiguration des Auftrags als
 * Auftragsparameter und stellt ein DOM zur Verfügung.
 * </p>
 * 
    ...
    public boolean spooler_init() {
        ...
        try {
            ...
            try { // to read stdout and stderr of this task and its subprocesses
                if (spooler_task != null && spooler_task.stdout_path() != null && spooler_task.stdout_path().length() > 0
                        && spooler_task.stderr_path() != null && spooler_task.stderr_path().length() > 0) {
    ...
    public void prepareAttributes() throws Exception {
        ...
        try { // to set processing attributes from parameters
            ...
            if (this.getParameters().value("scheduler_order_ignore_stderr") != null
                    && this.getParameters().value("scheduler_order_ignore_stderr").length() > 0) {
            ...
            if (this.getParameters().value("scheduler_order_ignore_error") != null
                    && this.getParameters().value("scheduler_order_ignore_error").length() > 0) {
            ...
            if (this.getParameters().value("scheduler_order_ignore_signal") != null
                    && this.getParameters().value("scheduler_order_ignore_signal").length() > 0) {
            ...
            try {
                if (this.getParameters().value("scheduler_order_timeout") != null
                        && this.getParameters().value("scheduler_order_timeout").length() > 0) {
    ...
    public void initConfiguration() throws Exception {
        ...
        if (spooler_job.order_queue() != null && spooler_task.order().params() != null) {
            if (spooler_task.order().params().value("configuration_path") != null
                    && spooler_task.order().params().value("configuration_path").length() > 0)
            ...
            if (spooler_task.order().params().value("configuration_file") != null
                    && spooler_task.order().params().value("configuration_file").length() > 0)
                        ...
                        if (nodeMap != null && nodeMap.getNamedItem("name") != null) {
                            ...
                            if (globalEnv || nodeMap.getNamedItem("env") != null) {
                                ...
                                boolean setEnv = globalEnv
                                        || (env.equalsIgnoreCase("yes") || env.equals("1") || env.equalsIgnoreCase("on") || env.equalsIgnoreCase("true"));
                                if (setEnv) {
                                    if (this.additional_envvars == null)
