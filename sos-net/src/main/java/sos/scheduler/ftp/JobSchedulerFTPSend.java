    ...
    public boolean spooler_process() {
        ...
        try {
                    ...
                    for (int i = 0; i < paramNames.length; i++) {
                        if (paramNames[i].startsWith("ftp_check_send_" + normalize(spooler_task.order().id()) + ".")) {
                            if ("0".equals(sosString.parseToString(spooler.var(paramNames[i])))) {
                                ...
                                String sRetry = sosString.parseToString(spooler.variables().var("cur_transfer_retry"
            ...
            try {
                if (parallelTransfer && !isFilePath) {
                    ...
                    if (!isJobchain()) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            spooler.variables().set_var("ftp_order", normalize(spooler_task.order().id()) + "." 
                                    + normalize(newOrder.id()) + "." + "0");
                            spooler.variables().set_var("ftp_check_send_" + normalize(spooler_task.order().id()) 
                                        + "." + normalize(newOrder.id()), "0");
                ...
                SOSConfiguration con = new SOSConfiguration(null, mapToProperties(schedulerParams), 
                        sosString.parseToString(schedulerParams.get(PARAMETER_SETTINGS)), sosString.parseToString(schedulerParams.get(PARAMETER_PROFILE)), 
                ...
                if (parallelTransfer && isFilePath && spooler_job.order_queue() != null) {
                    spooler.variables().set_var("ftp_check_send_" + normalize(params.var("ftp_parent_order_id")) + "."
            ...
            if (fileNotificationTo != null && fileNotificationTo.length() > 0) {
                if (fileNotificationSubject == null || fileNotificationSubject.length() == 0) {
                    if (spooler_job.order_queue() != null) {
                        fileNotificationSubject = "[info] Job Chain: " + spooler_task.order().job_chain().name() + ", Order: "
                                + spooler_task.order().id() + ", Job: " + spooler_job.name() + " (" + spooler_job.title() + "), Task: "
                                + spooler_task.id();
                    } else {
            ...
            if (fileZeroByteNotificationTo != null && fileZeroByteNotificationTo.length() > 0) {
                if (fileZeroByteNotificationSubject == null || fileZeroByteNotificationSubject.length() == 0) {
                    if (spooler_job.order_queue() != null) {
                        ...
                        fileZeroByteNotificationSubject = "[warning] Job: " + spooler_job.name() + " (" + spooler_job.title() + "), Task: "
                                + spooler_task.id();
                    }
