...
public class SOSSFTP implements SOSFileTransfer {
    ...
    private String resolvePathname(String pathname){
    	...
    	//if (!pathname.startsWith("/") && currentDirectory.length()>0){
			String slash = "";
			if (!currentDirectory.endsWith("/")) slash="/";
	...
	public void disconnect() throws IOException {
		reply ="disconnect OK";
		if (sftpClient !=null)try{
			sftpClient.close();
			sftpClient = null;
		} catch (Exception ex) {
			reply ="disconnect: "+ex;
		} // gracefully ignore this error
		if (sshConnection != null) try { 
			sshConnection.close(); 
			sshConnection=null; 
		} catch (Exception ex) {
			reply ="disconnect: "+ex;
	...
	public Vector nList(String pathname) throws Exception {
		...
		if (pathname.length()==0) pathname=".";
		if (!fileExists(pathname)) return rvVector;
		if (!isDirectory(pathname)){
			File remoteFile = new File(pathname);
	...
	private void nList(Vector rvVector, String workingDirectory,
    ...
    public static void test2() throws Exception{
		...
		/*
		String filename = "C:/scheduler/publickeys/mo";
    	String text = sos.util.SOSFile.readFile(new File(filename));
