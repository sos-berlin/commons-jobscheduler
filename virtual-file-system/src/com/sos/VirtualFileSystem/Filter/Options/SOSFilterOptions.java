	...
	public Vector<SOSNullFilter> getFilter() {
		...
		if (lstFilters == null) {
			...
			if (FilterSequence.isDirty() == true) {
				...
				if (exclude_lines_after.isDirty() || excludeLinesBefore.isDirty()) { // name: records
					SOSNullFilter objF = new SOSRecordsFilter(this);
					lstFilters.add(objF);
				}
				if (excludeEmptyLines.isDirty() || excludeLines.isDirty() || includeLines.isDirty()) { // name: excludeinclude
	...
	public SOSNullFilter getFilterInstance(final String pstrFilterName) {
		...
		for (enuFilterCodes enuFC : enuFilterCodes.values()) {
			if (enuFC.name().equalsIgnoreCase(pstrFilterName)) {
				switch (enuFC) {
					// TODO eine Klasse, die die Filter Instanz liefert und auch den Filter-Type liefert: stream, endpoint, startpoint, ... Ebenso für Validator einsetzen
					case dos2unix: //
