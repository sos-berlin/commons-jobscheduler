...
//	private static final String	FIELD_TRANSFER_TIMESTAMP	= "transfer_timestamp";
	...
	public void run() {
		...
		try {
			...
			boolean flgIncludeSubdirectories = objOptions.recursive.value();
			if (flgIncludeSubdirectories == true) {
				// TODO Das Erstellen des Verzeichnis muss eine separate Methode werden
				// ueberpruefen, ob das Verzeichnis auf den Target (-Server) existiert, wenn nicht dann soll das gleiche Verzeichnis generiert
				// werden
				if (objSourceFile.getParentVfs() != null && objSourceFile.getParentVfsFile().isDirectory()) {
					...
					if (subParent != null) {
						...
						// TODO nur feststellen, ob es den schon gibt. Neue Methode einfuehren, evtl. FileExists/Folderexists
						// String[] ftpFiles = objDataTargetClient.listNames(strTargetFolderName + "/" + subPath);
						// if (ftpFiles == null || ftpFiles.length == 0) {
						// File.separator fuehrte zu Unterverzeichnissen mit fuehrendem Backslash
