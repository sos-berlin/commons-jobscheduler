 ...
 * <p>
 * Diese Klasse reprüsentiert eine Datenstruktur für die Dateiverarbeitung von
 * lokalen und remote Dateien.
 * </p>
 * <p>
 * Verwendung findet diese Struktur beim download von Dateien, deren Name über
 * einen (regulüren) Ausdruck definiert ist und dabei dann mehr als eine Datei
 * relevant ist.
 * </p>
 * 
...
public class SOSFileListEntry extends SOSVfsMessageCodes implements Runnable, IJadeTransferDetailHistoryData /*
    ...
    /** Diese Routine prüft *nur* auf dem lokalen fileSystem und ist aus der
     * "Send"-Klasse kopiert. hier hat sie nichts zu suchen, da das polling für
     * dateien in der Engine erfolgen muss, unabhüngig davon, wo die Dateien
     * liegen. Es muü *immer* die Datasource gepollt werden.
     *
     * Auf alle Fülle muü hier über das Vfs auf die Datei(en) zugegriffen
     * werden. */
    public boolean CheckFileSizeIsChanging() throws Exception {
        ...
        try {
            ...
            // double nrOfTries = (objOptions.poll_timeout.value() * 60) /
            // delay;
            // int tries = 0;
            // boolean found = true;
            ...
            // // TODO über Option auf FileSize ausweichen, da MD5 bei groüen
            // dateien eine ziemliche Last erzeugt
            ...
            // }
            // if (found == false) {
            // message = SOSVfs_E_224.params(objOptions.poll_timeout.value(),
            // message);
    ...
    public void DeleteSourceFile() {
        // SOSFTP-152: allow source file deletion even if the filename is
        // changed by (replace/replacing)
    ...
    private boolean doTransfer(final ISOSVirtualFile source, final ISOSVirtualFile target) {
        @SuppressWarnings("unused")
        final String method = conClassName + "::doTransfer";
            ...
            synchronized (this) {
                ...
                if (source.getFileSize() <= 0) {
                    ...
                    while ((bytesTransferred = source.read(buffer)) != -1) {
                        ...
                        // TODO in case of wrong outputbuffer the handling of
                        // the error must be improved
    ...
    public void executeTFNPostCommnands() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::executeTFNPostCommnands";
    ...
    public String getFileName4ResultList() {
        @SuppressWarnings("unused")
        final String conMethodName = conClassName + "::getFileName4ResultList";
    ...
    // "guid;mandator;transfer_timestamp;pid;ppid;operation;localhost;localhost_ip;local_user;remote_host;remote_host_ip;remote_user;protocol;port;local_dir;remote_dir;local_filename;remote_filename;file_size;md5;status;last_error_message;log_filename";
    @Override
     ...
     * the target as the first filename After successfull completion of the
     * whole transfer, this name will be renamed to the final targetFileName.
     *
    ...
    public void setStatus(final enuTransferStatus peTransferStatus) {
        ...
        if (isNotEmpty(strM)) {
            ...
            // if (objOptions.transactional.value() == false && peTransferStatus
            // == enuTransferStatus.transferred) {
            // sendTransferHistory();
            // }
            // }
            switch (peTransferStatus) {
     ...
     *
     * \details Check, wether a source file exists.
     *
     * \return boolean */
    ...
    public Properties getFileAttributesAsProperties(HistoryRecordType recordType) {
        ...
        properties.put(FIELD_TRANSFER_TIMESTAMP, transfer_timestamp); // 3-
                                                                      // timestamp:
                                                                      // Zeitstempel
                                                                      // im
                                                                      // ISO-Format
        properties.put(FIELD_PID, pid); // 4- pid= Environment PID | 0 für
                                        // Windows
        properties.put(FIELD_PPID, ppid); // 5- ppid= Environment PPID | 0 für
                                          // Windows
        properties.put(FIELD_OPERATION, operation); // 6- operation:
                                                    // send|receive
        properties.put(FIELD_LOCALHOST, localhost); // 7- local host
        properties.put(FIELD_LOCALHOST_IP, localhost_ip); // 8- local host IP
                                                          // adresse
        properties.put(FIELD_LOCAL_USER, local_user); // 9- local user
        properties.put(FIELD_REMOTE_HOST, remote_host); // 10- remote host
        properties.put(FIELD_REMOTE_HOST_IP, remote_host_ip); // 11- remote host
                                                              // IP
    ...
    public String toCsv() {
        ...
        addCSv(properties.get(FIELD_TRANSFER_TIMESTAMP)); // 3- timestamp:
                                                          // Zeitstempel im
                                                          // ISO-Format
        addCSv(properties.get(FIELD_PID)); // 4- pid= Environment PID | 0 für
                                           // Windows
        addCSv(properties.get(FIELD_PPID)); // 5- ppid= Environment PPID | 0 für
                                            // Windows
