...
public class SOSFTPOptions extends SOSFtpOptionsSuperClass {
    ...
    public void CheckMandatory() {
        ...
        // TODO mandatory options in datatarget/datasource pruefen. Interface
        // erweitern um checkmandatory
        ...
        if (append_files.value() == true) {
            ...
            // TODO this check must be done at the implementation layer of the
            // protocol-class.
        ...
        if (file_path.isDirty()) {
            if (file_spec.isDirty()) {
                file_path.Value("");
                // String strT = getOptionNamesAsString(new SOSOptionElement[] {
                // file_path, file_spec });
                // throw new
                // JobSchedulerException(SOSVfsMessageCodes.SOSVfs_E_0030.params(strT));
            }
    ...
    private void checkReplaceAndReplacing(final SOSConnection2OptionsSuperClass pobjO) {
        if (pobjO.replacing.IsNotEmpty() && pobjO.replacement.IsNull()) {
            ...
            // throw new
            // JobSchedulerException(String.format(objMsg.getMsg(SOSVfs_E_0020),
            // replacing.getKey(), replacement.getKey()));
        }
        if (pobjO.replacing.IsEmpty() && pobjO.replacement.IsNotEmpty()) {
            // throw new
            // JobSchedulerException("SOSVfs-E-0000: parameter is missing for specified parameter [replacement]: [replacing]");
    ...
    private Properties getEnvVars() throws Exception {
        // TODO raus hier. die Routine ist zu generell und muss in die
        // Shell-Klasse
        try {
            propSOSFtpEnvironmentVars = new Properties();
            // TODO SchedulerParams hat hier nichts zu suchen. Die muessen
            // "von oben" ueber den JS-Adapter in die Klassen gereicht werden
            ...
            for (Object k : propAllEnvironmentVariables.keySet()) {
                ...
                // TODO obsolet. must come from the JSAdapter as a
                // Property-Collection
    ...
    @Override
    public void CommandLineArgs(final String pstrArgs) {
        // TODO split(" ") is buggy, wenn der Wert einer Option ein Leerzeichen
        // enthaelt.
        try {
    ...
    /** The Name of the Settings-File and the Name of the Profile (Section) has
     * to be defined by the Options "settings" and "profile" before entering
     * this method.
     * 
     * @return HashMap */
        ...
        try {
            ...
            for (Map.Entry<Object, Object> e : properties.entrySet()) {
                ...
                if (hasVariableToSubstitute(value) == true && gflgSubsituteVariables == true) {
                    ...
                    // TODO wrong place: has to come from the JS-Adapter as
                    // properties
    ...
    public String substituteVariables(String txt, final Properties prop, final String startPrefix, final String endPrefix) {
        try {
            for (Object k : prop.keySet()) {
                ...
                // logger.debug(String.format("search for  '%1$s' with value '%2$s'",
                // strSearchFor, strValue));
                int pos1 = -1;
                int pos2 = 0;
                while (true) {
                    ...
                    // logger.debug(String.format("found '%1$s'",
                    // strSearchFor));
                    ...
                    if (pos1 > -1 && pos2 > pos1) {
                        ...
                        // logger.debug(String.format("new Text = '%1$s'",
                        // txt));
                    }
                } // while
            } // for
     ...
     *
     * \details Returns true, if one or more file-/path-names explicit specified
    ...
    public String getDataSourceType() {
        ...
        if (operation.Value().equalsIgnoreCase(conOperationSEND)) {
            ...
            // strDataSourceType =
            // this.getConnectionOptions().Source().protocol.Value();
            // }
