...
public class SOSFTPOptions extends SOSFtpOptionsSuperClass {
	...
	public SOSFTPOptions(final SOSOptionTransferType.enuTransferTypes penuTransferType) {
		...
		case webdav:
			...
			this.getSource().authMethod.changeDefaults(enuAuthenticationMethods.url.text,
					enuAuthenticationMethods.url.text);
			this.getTarget().authMethod.changeDefaults(enuAuthenticationMethods.url.text,
					enuAuthenticationMethods.url.text);
		...
		case http:
			...
			this.getTarget().authMethod.changeDefaults(enuAuthenticationMethods.url.text,
					enuAuthenticationMethods.url.text);
			protocol.changeDefaults(SOSOptionTransferType.enuTransferTypes.http.getText(),
	...
	private void changeDefaults(final SOSOptionTransferType.enuTransferTypes penuTransferType,
			final SOSConnection2OptionsAlternate pobjOpt) {
		switch (penuTransferType) {
		case webdav:
		...
		case local:
		case file:
			pobjOpt.authMethod.changeDefaults(enuAuthenticationMethods.password.text,
					enuAuthenticationMethods.password.text);
	...
	public void adjustDefaults() {
		...
		setDefaultHostPort(this.getSource().getAlternatives().protocol, this.getSource().getAlternatives().port,
				this.getSource().getAlternatives().host);
		setDefaultHostPort(this.getTarget().getAlternatives().protocol, this.getTarget().getAlternatives().port,
				this.getTarget().getAlternatives().host);
		getDataSourceType();
		getDataTargetType();
	}
	...
	public void checkMandatory() {
		...
		if (replacing.IsEmpty() && replacement.isNotEmpty()) {
			throw new JobSchedulerException(
					SOSVfsMessageCodes.SOSVfs_E_0020.params(replacement.getKey(), replacing.getKey()));
		}
		if (appendFiles.value()) {
			...
			if (!"ftp".equalsIgnoreCase(protocol.getValue())) {
				throw new JobSchedulerException(
						SOSVfsMessageCodes.SOSVfs_E_0040.params(strAppendFilesKey, protocol.getValue()));
			}
		}
		if (this.getTarget().protocol.isDirty() && !protocol.isDirty()) {
		...
		if (filePath.IsEmpty() && sourceDir.IsEmpty() && this.getSource().directory.IsEmpty()
				&& fileListName.IsEmpty()) {
			throw new JobSchedulerException(
					String.format("SOSVfs-E-0000: one of these parameters must be specified: '%1$s', '%2$s', '%3$s'",
	...
	private void checkReplaceAndReplacing(final SOSConnection2OptionsSuperClass pobjO) {
		...
		if (pobjO.replacing.IsEmpty() && pobjO.replacement.isNotEmpty()) {
			throw new JobSchedulerException(
					SOSVfsMessageCodes.SOSVfs_E_0020.params(pobjO.replacement.getKey(), pobjO.replacing.getKey()));
		}
	}
	...
	private void setDefaultAuth(final SOSOptionTransferType pobjTransferTyp,
			final SOSConnection2OptionsAlternate objConn) {
		enuTransferTypes transferType = pobjTransferTyp.getEnum();
		if ((transferType == enuTransferTypes.http || transferType == enuTransferTypes.https
				|| transferType == enuTransferTypes.webdav) && !objConn.authMethod.isDirty()
				&& !objConn.sshAuthMethod.isDirty()) {
	...
	private void setDefaultHostPort(final SOSOptionTransferType pobjTransferTyp, final SOSOptionPortNumber pobjPort,
		...
		case local:
			...
			if (pobjHost.isNotDirty() || "localhost".equalsIgnoreCase(pobjHost.getValue())
					|| "127.0.0.1".equalsIgnoreCase(pobjHost.getValue())) {
	...
	public void setAllOptions2(HashMap<String, String> params) {
		...
		if (!flgSettingsFileProcessed && !flgReadSettingsFileIsActive && params.containsKey("settings")
	...
	public String substituteVariables(String txt, final Properties prop, final String startPrefix,
