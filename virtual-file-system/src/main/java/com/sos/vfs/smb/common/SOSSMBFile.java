...
public class SOSSMBFile extends SOSCommonProviderFile {
    ...
    public OutputStream getFileOutputStream() {
        try {
            if (getOutputStream() == null) {
                ...
                ISOSSMB handler = (ISOSSMB) getProvider();
    ...
    public long setModificationDateTime(final long timeStamp) {
        try {
            ((ISOSSMB) getProvider()).setModificationTimeStamp(fileName, timeStamp);
            return timeStamp;
        } catch (Exception e) {
    ...
    public long getModificationDateTime() {
        try {
            return ((ISOSSMB) getProvider()).getModificationTimeStamp(fileName);
        } catch (Exception e) {
            LOGGER.error(e.getMessage(), e);
            return -1L;
        }
