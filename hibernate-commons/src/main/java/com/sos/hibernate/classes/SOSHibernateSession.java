...
public class SOSHibernateSession implements Serializable {
	...
	/**
	 * use factory.openSession(), factory.openStatelessSession() or
	...
	protected void openSession() throws SOSHibernateOpenSessionException {
		String method = SOSHibernate.getMethodName(logIdentifier, "openSession");
		if (currentSession != null) {
			logDebug(String.format("%s close currentSession", method));
			closeSession();
		}
		try {
			...
			logDebug(String.format("%s %s, autoCommit=%s", method, sessionName, autoCommit));
	...
	public void beginTransaction() throws SOSHibernateException {
		String method = SOSHibernate.getMethodName(logIdentifier, "beginTransaction");
		if (autoCommit) {
			logDebug(String.format("%s skip (autoCommit is true)", method));
			return;
		}
		if (currentSession == null) {
	...
	public void clearSession() throws SOSHibernateException {
		...
		logDebug(SOSHibernate.getMethodName(logIdentifier, "clearSession"));
		try {
			if (!isStatelessSession) {
				Session session = (Session) currentSession;
				session.clear();
	...
	public void close() {
		logDebug(SOSHibernate.getMethodName(logIdentifier, "close"));
		closeTransaction();
		closeSession();
	}
	...
	public void commit() throws SOSHibernateException {
		String method = SOSHibernate.getMethodName(logIdentifier, "commit");
		if (autoCommit) {
			logDebug(String.format("%s skip (autoCommit is true)", method));
			return;
		}
	...
	public void connect() throws SOSHibernateConfigurationException, SOSHibernateOpenSessionException {
		openSession();
		String connFile = (factory.getConfigFile().isPresent())
				? factory.getConfigFile().get().toAbsolutePath().toString()
				: "without config file";
		int isolationLevel = getFactory().getTransactionIsolation();
		logDebug(String.format("%s autocommit=%s, transaction isolation=%s, %s",
				SOSHibernate.getMethodName(logIdentifier, "connect"), getFactory().getAutoCommit(),
				SOSHibernateFactory.getTransactionIsolationName(isolationLevel), connFile));
	}

	/**
	 * @throws SOSHibernateException
	 *             : SOSHibernateInvalidSessionException,
	 *             SOSHibernateLockAcquisitionException, SOSHibernateQueryException
	 */
	...
	public <T> NativeQuery<T> createNativeQuery(String sql, Class<T> entityClass) throws SOSHibernateException {
		...
		logTrace(String.format("%s[sql][%s]", SOSHibernate.getMethodName(logIdentifier, "createNativeQuery"), sql));
	...
	public <T> Query<T> createQuery(String hql, Class<T> entityClass) throws SOSHibernateException {
		...
		logTrace(String.format("%s[hql][%s]", SOSHibernate.getMethodName(logIdentifier, "createQuery"), hql));
		Query<T> q = null;
		try {
			if (isStatelessSession) {
				if (entityClass == null) {
	...
	public <T> T get(Class<T> entityClass, Serializable id) throws SOSHibernateException {
		...
		logDebug(String.format("%s entityClass=%s, id=%s", SOSHibernate.getMethodName(logIdentifier, "get"),
			...
			if (isStatelessSession) {
				...
				try {
					...
					throw new SOSHibernateConnectionException(
							"can't get the SQL connection from the StatelessSessionImpl(NullPointerException)");
				}
			} else {
				...
				try {
					...
					throw new SOSHibernateConnectionException(
							"can't get the SQL connection from the SessionImpl(NullPointerException)");
				}
			}
		} catch (IllegalStateException e) {
	 ...
	 * 
	 * 
	 * setResultTransformer is deprecated (see below), but currently without
	 * alternative
	 * 
	 ...
	 *             SOSHibernateLockAcquisitionException, SOSHibernateQueryException
	 */
	public <T> List<Map<String, String>> getResultListAsStringMaps(NativeQuery<T> nativeQuery)
			throws SOSHibernateException {
	 ...
	 * 
	 * Map key - column name (lower case), Map value - string representation (null
	 ...
	 */
	@SuppressWarnings({ "deprecation", "unchecked" })
	public <T> List<Map<String, String>> getResultListAsStringMaps(NativeQuery<T> nativeQuery, String dateTimeFormat)
			throws SOSHibernateException {
		if (nativeQuery == null) {
			throw new SOSHibernateQueryException("nativeQuery is NULL");
		}
	 ...
	 * 
	 * difference to Query.getSingleResult - not throw NoResultException, return
		...
		if (result != null) {
			if (query instanceof NativeQuery<?>) {
				if (result instanceof Object[]) {
					throw new SOSHibernateQueryNonUniqueResultException(
							"query return a row and not a unique field result", query);
				}
			} else {
				if (result.getClass().getAnnotation(Entity.class) != null) {
					throw new SOSHibernateQueryNonUniqueResultException(
							"query return an entity object and not a unique field result", query);
				}
			}
			return result;
		}
	...
	public void reopen() throws SOSHibernateOpenSessionException {
		logDebug(String.format("%s isStatelessSession=%s", SOSHibernate.getMethodName(logIdentifier, "reopen"),
				isStatelessSession));
		closeSession();
		openSession();
	}
	...
	public void rollback() throws SOSHibernateException {
		String method = SOSHibernate.getMethodName(logIdentifier, "rollback");
		if (autoCommit) {
			logDebug(String.format("%s skip (autoCommit is true)", method));
			return;
		}
				...
				try {
					...
					if (id == null) {
						throw new SOSHibernateException(String.format(
								"not found @Id annotated public getter method [%s]", item.getClass().getName()));
					}
	...
	public void sessionDoWork(Work work) throws SOSHibernateException {
		...
		logDebug(SOSHibernate.getMethodName(logIdentifier, "sessionDoWork"));
		try {
			if (!isStatelessSession) {
				Session session = (Session) currentSession;
				session.doWork(work);
	...
	public void setAutoCommit(boolean val) {
		logDebug(String.format("%s %s", SOSHibernate.getMethodName(logIdentifier, "setAutoCommit"), val));
		autoCommit = val;
	}
	...
	private void closeSession() {
		logDebug(String.format("%s", SOSHibernate.getMethodName(logIdentifier, "closeSession")));
		try {
			if (currentSession != null) {
				if (isStatelessSession) {
					StatelessSession s = (StatelessSession) currentSession;
	...
	private void closeTransaction() {
		try {
			if (currentSession != null) {
				Transaction tr = getTransaction();
				if (tr != null) {
					logDebug(String.format("%s[rollback]%s",
							SOSHibernate.getMethodName(logIdentifier, "closeTransaction"), tr.getStatus()));
					tr.rollback();
				} else {
					logDebug(String.format("%s[skip][rollback]transaction is null",
							SOSHibernate.getMethodName(logIdentifier, "closeTransaction")));
				}
			}
		} catch (Throwable ex) {
			//
	...
	private void onOpenSession() throws SOSHibernateOpenSessionException {
		...
		try {
			if (getFactory().getDbms().equals(SOSHibernateFactory.Dbms.MSSQL)) {
				String value = getFactory().getConfiguration().getProperties()
						.getProperty(SOSHibernate.HIBERNATE_SOS_PROPERTY_MSSQL_LOCK_TIMEOUT);
	...
	private void throwException(PersistenceException cause, SOSHibernateException ex) throws SOSHibernateException {
		...
		while (e != null) {
			if (e instanceof JDBCConnectionException) {
			...
			} else if (e instanceof SQLNonTransientConnectionException) {// can occur without hibernate
																			// JDBCConnectionException
				throw new SOSHibernateInvalidSessionException((SQLNonTransientConnectionException) e,
						ex.getStatement());
				...
				if (getFactory().getDbms().equals(SOSHibernateFactory.Dbms.MYSQL)) {
					// MySQL with the mariadb driver not throws a specific exception - check error
					// code
