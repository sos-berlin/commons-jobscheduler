...
public class JobSchedulerJobStreamsEventHandler extends LoopEventHandler {
	...
	public void onActivate(Notifier notifier) {
		...
		try {
			...
			conditionResolver = new JSConditionResolver(sosHibernateSession, this.getXmlCommandExecutor(),
					this.getSettings());
	...
	private void performTaskEnd(SOSHibernateSession sosHibernateSession, TaskEndEvent taskEndEvent) throws Exception {
		...
		boolean dbChange = conditionResolver.resolveOutConditions(taskEndEvent.getReturnCode(),
		...
		try {
			...
			do {
				...
				for (JSInCondition jsInCondition : listOfValidatedInconditions) {
					publishCustomEvent(CUSTOM_EVENT_KEY, CustomEventType.InconditionValidated.name(),
			...
			for (JsonValue entry : events) {
				if (entry != null) {
					...
					case "OrderFinished":
						...
						conditionResolver.checkHistoryCache(
					...
					case "TaskClosed":
						...
						conditionResolver.enableInconditionsForJob(getSettings().getSchedulerId(),
					...
					case "VariablesCustomEvent":
						...
						case "AddEvent":
							...
							publishCustomEvent(CUSTOM_EVENT_KEY, CustomEventType.EventCreated.name(),
									customEvent.getEvent());
							addQueuedEvents.handleEventlistBuffer(conditionResolver.getNewJsEvents());
						...
						case "RemoveEvent":
							...
							publishCustomEvent(CUSTOM_EVENT_KEY, CustomEventType.EventRemoved.name(),
									customEvent.getEvent());
