						...
						try {
							if (sosHibernateSession != null) {
								...
								LOGGER.debug("Set next start for " + dbItemJobStreamStarter.getJobStream() + "."
										+ dbItemJobStreamStarter.getTitle() + " to "
		...
		public void run() {
			...
			if (initEvents) {
				...
				createNewModel = CompletableFuture.supplyAsync(() -> {
					...
					/*
					 * try { assignNewModel2(jsConditionResolver); } catch (SOSHibernateException e)
					 * { LOGGER.error(e.getMessage(), e); }
					 */
				});
			...
			try {
				...
				if (ACTIVE.equals(nextStarter.getItemJobStreamStarter().getState())) {
					...
					LOGGER.debug("Starter: " + nextStarter.getItemJobStreamStarter().getId() + "."
							+ nextStarter.getItemJobStreamStarter().getTitle() + " is not active. Not started");
				}
						...
						case "StartJob":
							...
							if (dbItemJobStreamHistory != null) {
								...
								if (jsJobStream != null) {
									...
									StartJobReturn startJobReturn = jsInConditionCommand.startJob(
