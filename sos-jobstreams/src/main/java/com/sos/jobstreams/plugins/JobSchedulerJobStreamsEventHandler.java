...
public class JobSchedulerJobStreamsEventHandler extends LoopEventHandler {
	...
	public void resetJobStreamCheckIntervalTimer() {
		...
		if (Constants.jobstreamCheckInterval > 0) {
			jobStreamCheckIntervalTimer.schedule(new JobStreamCheckIntervalTask(),
		...
		try {
			...
			for (DBItemJobStreamStarter dbItemJobStreamStarter : listOfStarter) {
				...
				if (dbItemJobStreamStarter.getRunTime() != null && !"".equals(dbItemJobStreamStarter.getRunTime())) {
					if (dbItemJobStreamStarter.getNextStart() == null
							|| dbItemJobStreamStarter.getNextStart().before(now)) {
						Date nextStart = dbLayerJobStreamStarters.getNextStartTime(objectMapper,
								getSettings().getTimezone(), dbItemJobStreamStarter.getRunTime());
						if (nextStart != null) {
							dbItemJobStreamStarter.setNextStart(nextStart);

							dbLayerJobStreamStarters.updateNextStart(dbItemJobStreamStarter);
							LOGGER.debug("Refreshed next start for " + dbItemJobStreamStarter.getJobStream() + "."
									+ dbItemJobStreamStarter.getTitle() + " to " + nextStart);
			...
			do {
				if (this.getConditionResolver().getJsJobStreams() != null) {
					...
					if (nextStarter != null) {
						...
						LOGGER.debug(
								"Next start:" + nextStarter.getAllJobNames() + " at " + nextStarter.getNextStart());

						try {
							if (sosHibernateSession != null) {

								DBLayerJobStreamStarters dbLayerJobStreamStarters = new DBLayerJobStreamStarters(
										sosHibernateSession);
								sosHibernateSession.beginTransaction();
								...
								dbLayerJobStreamStarters.updateNextStart(dbItemJobStreamStarter);
								nextStarter.setItemJobStreamStarterNoSchedule(dbLayerJobStreamStarters
										.getJobStreamStartersDbItem(dbItemJobStreamStarter.getId()));

								sosHibernateSession.commit();
								LOGGER.debug("Set next start for " + dbItemJobStreamStarter.getJobStream() + "."
										+ dbItemJobStreamStarter.getStarterName() + " to "
										+ dbItemJobStreamStarter.getNextStart());
								addPublishEventOrder(CUSTOM_EVENT_KEY, CustomEventType.StartTime.name(),
										String.valueOf(nextStarter.getItemJobStreamStarter().getId()));

							}
						} catch (SOSHibernateException e) {
							LOGGER.error("Could not update next start time for starter: "
									+ nextStarter.getItemJobStreamStarter().getId() + ":"
	...
	private JSConditionResolver reInitModel() {
		...
		try {
			...
			LOGGER.debug("acquire: eventHandlerSemaphore");
			...
			LOGGER.debug("release: eventHandlerSemaphore");
	...
	public class JobStreamCheckIntervalTask extends TimerTask {
		...
		public void run() {
			...
			try {
				...
				ReportTaskExecutionsDBLayer reportTaskExecutionsDBLayer = new ReportTaskExecutionsDBLayer(
						sosHibernateSession);
				reportTaskExecutionsDBLayer.resetFilter();
				reportTaskExecutionsDBLayer.getFilter().setSchedulerId(getSettings().getSchedulerId());

				for (Entry<UUID, DBItemJobStreamHistory> entry : getConditionResolver().getListOfHistoryIds()
						.entrySet()) {
					if (getConditionResolver().getJobStreamContexts() == null) {
						continue;
					}
					if ((getConditionResolver().getJobStreamContexts().getTaskIdsOfContext(entry.getKey()) != null)
							&& (getConditionResolver().getJobStreamContexts().getTaskIdsOfContext(entry.getKey())
									.size() > 0)) {
						LOGGER.debug("Select open tasks for " + entry.getKey());
						LOGGER.debug("Taskids in filter:" + getConditionResolver().getJobStreamContexts()
								.getTaskIdsOfContext(entry.getKey()).size());
						reportTaskExecutionsDBLayer.getFilter().setTaskIds(
								getConditionResolver().getJobStreamContexts().getTaskIdsOfContext(entry.getKey()));
						if (isDebugEnabled) {
							LOGGER.debug(SOSString.toString(reportTaskExecutionsDBLayer.getFilter()));
							for (Long taskId : getConditionResolver().getJobStreamContexts()
									.getTaskIdsOfContext(entry.getKey())) {
						...
						for (DBItemReportTask dbItemReportTask : listOfOpenTasks) {
							if (dbItemReportTask.getEndTime() != null) {
								if (isDebugEnabled) {
									LOGGER.debug("Task " + dbItemReportTask.getHistoryId()
											+ " is terminated but not resolved");
								}
		...
		public void run() {
			...
			try {
				...
				if (ACTIVE.equals(nextStarter.getItemJobStreamStarter().getState())) {
					...
					LOGGER.debug("Starter: " + nextStarter.getItemJobStreamStarter().getId() + "."
							+ nextStarter.getItemJobStreamStarter().getStarterName() + " is not active. Not started");
				}
				...
				if (delay >= 0) {
					...
					if (sosHibernateSession != null) {
						...
						dbItemJobStreamStarter.setNextStart(new Date(next));
						dbLayerJobStreamStarters.updateNextStart(dbItemJobStreamStarter);
						nextStarter.setItemJobStreamStarterNoSchedule(
								dbLayerJobStreamStarters.getJobStreamStartersDbItem(dbItemJobStreamStarter.getId()));

						sosHibernateSession.commit();

						LOGGER.debug("Set next start for " + dbItemJobStreamStarter.getJobStream() + "."
								+ dbItemJobStreamStarter.getTitle() + " to " + dbItemJobStreamStarter.getNextStart());
						addPublishEventOrder(CUSTOM_EVENT_KEY, CustomEventType.StartTime.name(),
								String.valueOf(nextStarter.getItemJobStreamStarter().getId()));
	...
	private void reInitConsumedInConditions()
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException, Exception {
		try {
	...
	private void startJobs(JSJobStreamStarter jobStreamStarter, UUID contextId) throws Exception {
		...
		try {
			...
			JSJobStream jobStream = getConditionResolver().getJsJobStreams()
					.getJobStream(jobStreamStarter.getItemJobStreamStarter().getJobStream());

			LOGGER.debug(String.format("Adding history entry with context-id %s to jobStream %s",
					dbItemJobStreamHistory.getContextId(), jobStream.getJobStream()));
			jobStream.getJsHistory().addHistoryEntry(historyEntry, sosHibernateSession);
			sosHibernateSession.commit();

			LOGGER.debug("release:1a resolveConditionsSemaphore");
			...
			for (JobStarterOptions handledJob : listOfHandledJobs) {
				if (handledJob.isSkipped() && !handledJob.isSkipOutCondition()) {
					...
					LOGGER.debug(
							String.format("Job %s skipped. Job run will be simulated with rc=0", handledJob.getJob()));
					performTaskEnd(sosHibernateSession, taskEndEvent);

				} else {
					sosHibernateSession.beginTransaction();
					getConditionResolver().getJobStreamContexts().addTaskToContext(contextId,
							super.getSettings().getSchedulerId(), handledJob, sosHibernateSession);
					sosHibernateSession.commit();
				}
			}
			Map<String, String> values = new HashMap<String, String>();
			values.put(CustomEventType.JobStreamStarted.name(),
					jobStream.getJobStream() + "." + jobStreamStarter.getItemJobStreamStarter().getTitle());
	...
	public void onActivate(Notifier notifier) {
		...
		try {
			...
			EventType[] observedEventTypes = new EventType[] { EventType.TaskClosed, EventType.TaskEnded,
					EventType.VariablesCustomEvent, EventType.OrderFinished };
	...
	private boolean performTaskEnd(SOSHibernateSession sosHibernateSession, TaskEndEvent taskEndEvent)
			throws Exception {
		LOGGER.debug("TaskEnded event to be executed:" + taskEndEvent.getTaskId() + " " + taskEndEvent.getJobPath());
		boolean dbChanged = false;
		getConditionResolver().checkHistoryCache(sosHibernateSession, taskEndEvent.getJobPath(),
				taskEndEvent.getReturnCode());
		LOGGER.debug("acquire:2 resolveConditionsSemaphore");
		resolveConditionsSemaphore.acquire();
		try {
			dbChanged = getConditionResolver().resolveOutConditions(sosHibernateSession, taskEndEvent,
					getSettings().getSchedulerId(), taskEndEvent.getJobPath());
			...
			if (getConditionResolver().getJobStreamContexts().getTaskIdsOfContext(contextId) != null) {
				getConditionResolver().getJobStreamContexts().getTaskIdsOfContext(contextId)
						.remove(taskEndEvent.getTaskIdLong());
			}

			if (contextId != null) {

				getConditionResolver().enableInconditionsForJob(getSettings().getSchedulerId(),
						taskEndEvent.getJobPath(), contextId);
				CheckRunningResult checkRunningResult = getConditionResolver().checkRunning(sosHibernateSession,
						contextId);
		...
		if (!getConditionResolver().getNewJsEvents().isEmpty()
				|| !getConditionResolver().getRemoveJsEvents().isEmpty()) {
	...
	private String timeZoneFromInstances(SOSHibernateSession sosHibernateSesssion)
			throws DBInvalidDataException, DBConnectionRefusedException {
		InventoryInstancesDBLayer inventoryInstancesDBLayer = new InventoryInstancesDBLayer(sosHibernateSesssion);

		List<DBItemInventoryInstance> l = inventoryInstancesDBLayer
				.getInventoryInstancesBySchedulerId(getSettings().getSchedulerId());
		if (l.size() > 0) {
	...
	private void resolveInConditions(SOSHibernateSession sosHibernateSession, UUID forInstance) throws Exception {
		...
		try {
			...
			do {
				...
				List<JSInCondition> listOfValidatedInconditions = getConditionResolver()
						.resolveInConditions(sosHibernateSession, forInstance);
				LOGGER.debug("release:3a resolveConditionsSemaphore");
				resolveConditionsSemaphore.release();
				...
				if (listOfValidatedInconditions != null) {
					...
					for (JSInCondition jsInCondition : listOfValidatedInconditions) {
						...
						if (!jsInCondition.isSkipOutCondition()) {
							for (JSInConditionCommand inConditionCommand : jsInCondition
									.getListOfInConditionCommand()) {
								LOGGER.debug("isExecuted:" + inConditionCommand.getCommand() + "-->"
										+ inConditionCommand.isExecuted());
								if (!inConditionCommand.isExecuted()) {
									...
									LOGGER.debug(String.format("Job %s skipped. Job run will be simulated with rc=0",
											jsInCondition.getJob()));
			...
			LOGGER.debug("release:3b resolveConditionsSemaphore");
			resolveConditionsSemaphore.release();
		} finally {
		}
	}
	...
	private void execute(boolean onNonEmptyEvent, Long eventId, JsonArray events) throws InterruptedException {
		...
		try {
			...
			if (this.listOfConditionResolver == null || getConditionResolver() == null) {
				...
				createConditionResolver();
				try {
					LOGGER.debug("acquire:4 resolveConditionsSemaphore");
					resolveConditionsSemaphore.acquire();
					getConditionResolver().initComplete(sosHibernateSession);
					this.resetNextJobStartTimer();
					...
					LOGGER.debug("release:4 resolveConditionsSemaphore");
					resolveConditionsSemaphore.release();
				}
			}
			...
			for (JsonValue entry : events) {
				if (entry != null) {
					...
					case "OrderFinished":
						...
						getConditionResolver().checkHistoryCache(sosHibernateSession, orderFinishedEvent.getJobChain(),
								null);
						getConditionResolver().checkHistoryCache(sosHibernateSession,
								orderFinishedEvent.getJobChain() + "(" + orderFinishedEvent.getOrderId() + ")", null);
						break;
					case "TaskEnded":
						LOGGER.debug("Event to be executed: " + jobSchedulerEvent.getType());
						...
						taskEndUuid = getConditionResolver().getJobStreamContexts()
								.getContext(taskEndEvent.getTaskIdLong());
					...
					case "VariablesCustomEvent":
						...
						case "StartJobStream":
							LOGGER.debug("VariablesCustomEvent event to be executed: " + customEvent.getKey());
							JSJobStreamStarter jsJobStreamStarter = getConditionResolver().getListOfJobStreamStarter()
									.get(customEvent.getJobStreamStarterId());
							if (jsJobStreamStarter != null) {
								...
								LOGGER.warn("Could not find JobStream starter with id: "
										+ customEvent.getJobStreamStarterId());
							}
						...
						case "StartJob":
							...
							if (dbItemJobStreamHistory != null) {
								...
								JSJobStream jsJobStream = getConditionResolver().getJsJobStreams()
										.getJobStream(jobStreamId);
								if (jsJobStream != null) {
									...
									for (Entry<String, String> param : customEvent.getParameters().entrySet()) {
										getConditionResolver().getListOfParameters().get(contextId).put(param.getKey(),
												param.getValue());
									}
									StartJobReturn startJobReturn = jsInConditionCommand.startJob(contextId,
											this.getXmlCommandExecutor(), inCondition,
											getConditionResolver().getListOfParameters().get(contextId));
									sosHibernateSession.beginTransaction();
									getConditionResolver().handleStartedJob(contextId, sosHibernateSession,
											startJobReturn, inCondition);
									sosHibernateSession.commit();

								} else {
									LOGGER.warn(
											"Could not start task. JobStream with id " + jobStreamId + " not found.");
								}
							} else {
						...
						case "AddEvent":
							...
							List<DBItemJobStreamHistory> lHistoryAdded = dbLayerJobStreamHistory
									.getJobStreamHistoryList(filterJobStreamHistory, 0);
							if (lHistoryAdded.size() <= 0) {
								LOGGER.warn("Could not add Event " + customEvent.getEvent() + " as session "
										+ customEvent.getSession() + " has not been found");
							} else {
								...
								try {
									...
									LOGGER.warn("could not add event " + event.getEvent()
											+ ": NumberFormatException with -> " + event.getOutConditionId());
								}
								...
								String path = "";
								if ((getConditionResolver().getJsJobStreams() != null) && (getConditionResolver()
										.getJsJobStreams().getJobStreamByName(customEvent.getJobStream()) != null)) {
									path = Paths
											.get(getConditionResolver().getJsJobStreams()
													.getJobStreamByName(customEvent.getJobStream()).getFolder())
											.toString().replace('\\', '/');
								...
								if (!getConditionResolver().getNewJsEvents().isEmpty()
										&& !this.addQueuedEvents.isEmpty()) {
									this.addQueuedEvents.storetoDb(sosHibernateSession,
											getConditionResolver().getJsEvents());
									reInitConsumedInConditions();
								}
								resolveInConditions = true;
							}
						...
						case "RemoveEvent":
							LOGGER.debug("VariablesCustomEvent event to be executed: " + customEvent.getKey() + " --> "
									+ customEvent.getEvent());
							filterJobStreamHistory.addContextId(customEvent.getSession());
							List<DBItemJobStreamHistory> lHistoryDeleted = dbLayerJobStreamHistory
									.getJobStreamHistoryList(filterJobStreamHistory, 0);
							if (lHistoryDeleted.size() <= 0) {
								LOGGER.warn("Could not remove Event " + customEvent.getEvent() + " as session "
										+ customEvent.getSession() + " has not been found");
							} else {
								JSEvent event = new JSEvent();
								event.setCreated(new Date());
								...
								delQueuedEvents.handleEventlistBuffer(getConditionResolver().getRemoveJsEvents());
								if (!getConditionResolver().getRemoveJsEvents().isEmpty()
										&& !this.delQueuedEvents.isEmpty()) {
