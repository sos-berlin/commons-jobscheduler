...
public class JSInCondition implements IJSJobConditionKey, IJSCondition {
	...
	protected void markAsConsumed(SOSHibernateSession sosHibernateSession, UUID contextId)
	...
	public boolean isConsumed(UUID contextId) {
		LOGGER.debug("check consumed " + this.getExpression() + " is consumed for context " + contextId + " ---> "
				+ consumedForContext.contains(contextId));
	...
	public StartJobReturn executeCommand(SOSHibernateSession sosHibernateSession, UUID contextId,
			Map<String, String> listOfParameters, SchedulerXmlCommandExecutor schedulerXmlCommandExecutor)
