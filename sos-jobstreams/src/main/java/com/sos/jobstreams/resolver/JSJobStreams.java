...
public class JSJobStreams {
	...
	public void setListOfJobStreams(EventHandlerSettings settings, List<DBItemJobStream> listOfJobStreams,
			Map<Long, JSJobStreamStarter> listOfJobStreamStarterGlobal, SOSHibernateSession sosHibernateSession)
		...
		for (DBItemJobStream itemJobStream : listOfJobStreams) {
			...
			List<DBItemJobStreamStarter> listOfJobStreamStarters = dbLayerJobStreamStarters
					.getJobStreamStartersList(filterJobStreamStarters, 0);
			jsStreamStream.setJobStreamStarters(settings, listOfJobStreamStarters, listOfJobStreamStarterGlobal,
					sosHibernateSession);
	...
	public JSJobStreamStarter getNextStarter() {
		...
		for (JSJobStream jsJobStream : listOfJobStreams.values()) {
			...
			for (JSJobStreamStarter jsJobStreamStarter : jsJobStream.getListOfJobStreamStarter()) {
				...
				if (next == null || (jsJobStreamStarter.getNextStartFromList() != null)
						&& (jsJobStreamStarter.getNextStartFromList() == next
		...
		if (nextJsJobStreamStarter != null) {
			LOGGER.debug("--> Next starter ist: " + nextJsJobStreamStarter.getItemJobStreamStarter().getTitle() + " at "
					+ nextJsJobStreamStarter.getNextStart());
		} else {
		...
		for (JSJobStream jsJobStream : listOfJobStreams.values()) {
			for (JSJobStreamStarter jsJobStreamStarter : jsJobStream.getListOfJobStreamStarter()) {
				LOGGER.trace(jsJobStream.getJobStream() + "." + jsJobStreamStarter.getItemJobStreamStarter().getTitle()
						+ " last start: " + new Date(jsJobStreamStarter.getLastStart()));
			}
	...
	public JSJobStreamStarter reInitLastStart(JSJobStreamStarter nextStarter) {
		if (nextStarter != null) {
			for (JSJobStream jsJobStream : listOfJobStreams.values()) {
				for (JSJobStreamStarter jsJobStreamStarter : jsJobStream.getListOfJobStreamStarter()) {
					if (jsJobStreamStarter.getItemJobStreamStarter().getId() == nextStarter.getItemJobStreamStarter()
							.getId()) {
						LOGGER.trace(jsJobStream.getJobStream() + "." + jsJobStreamStarter.getItemJobStreamStarter().getTitle()
								+ " last start: reinit " + new Date(jsJobStreamStarter.getLastStart()) + " --> " + new Date(nextStarter.getLastStart()));
