...
public class JSConditionResolver {
	...
	public void initComplete(SOSHibernateSession sosHibernateSession)
	...
	public void initPartial(SOSHibernateSession sosHibernateSession)
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException, Exception {
		init(sosHibernateSession, false);
	}

	private void reInit(SOSHibernateSession sosHibernateSession, boolean complete)
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException,
			SOSHibernateException, SOSInvalidDataException, DOMException, ParseException, TransformerException {
	...
	public void reInitComplete(SOSHibernateSession sosHibernateSession)
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException, Exception {
		reInit(sosHibernateSession, true);
	}

	public void reInitPartial(SOSHibernateSession sosHibernateSession)
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException,
			SOSHibernateException, SOSInvalidDataException, DOMException, ParseException, TransformerException {
	...
	private void initJobInConditions(SOSHibernateSession sosHibernateSession) throws SOSHibernateException {
		...
		List<DBItemJobStreamHistory> listOfRunningJobStreams = dbLayerJobStreamHistory
				.getJobStreamHistoryList(filterJobStreamHistory, 0);
		...
		for (DBItemJobStreamHistory dbItemJobStreamHistory : listOfRunningJobStreams) {
			...
			DBLayerConsumedInConditions dbLayerCoumsumedInConditions = new DBLayerConsumedInConditions(
					sosHibernateSession);
			List<DBItemConsumedInCondition> l = dbLayerCoumsumedInConditions
					.getConsumedInConditionsList(filterConsumedInConditions, 0);
			for (DBItemConsumedInCondition dbItemConsumedInCondition : l) {
				if (mapOfConsumedInCondition.get(dbItemConsumedInCondition.getInConditionId()) == null) {
					mapOfConsumedInCondition.put(dbItemConsumedInCondition.getInConditionId(),
							new ArrayList<DBItemConsumedInCondition>());
				}
				mapOfConsumedInCondition.get(dbItemConsumedInCondition.getInConditionId())
						.add(dbItemConsumedInCondition);
			}
		}
		...
		List<DBItemInConditionWithCommand> listOfInConditions = dbLayerInConditions
				.getInConditionsList(filterInConditions, 0);
		for (DBItemInConditionWithCommand itemInConditionWithCommand : listOfInConditions) {
			...
			if (itemInCondition.getId() != null && mapOfConsumedInCondition.get(itemInCondition.getId()) != null) {
				for (DBItemConsumedInCondition dbItemConsumedInCondition : mapOfConsumedInCondition
						.get(itemInCondition.getId())) {
	...
	private void initJobstreamContext(SOSHibernateSession sosHibernateSession) throws SOSHibernateException {
		...
		filterJobStreamHistory.setRunning(true);
		DBLayerJobStreamHistory dbLayerJobStreamHistory = new DBLayerJobStreamHistory(sosHibernateSession);
		DBLayerJobStreamsTaskContext dbLayerJobStreamsTaskContext = new DBLayerJobStreamsTaskContext(
				sosHibernateSession);
		List<DBItemJobStreamHistory> listOfRunningStreams = dbLayerJobStreamHistory
				.getJobStreamHistoryList(filterJobStreamHistory, 0);
		for (DBItemJobStreamHistory dbItemJobStreamHistory : listOfRunningStreams) {
			...
			List<DBItemJobStreamTaskContext> listOfTaskContext = dbLayerJobStreamsTaskContext
					.getJobStreamStarterJobsList(filterTaskContext, 0);
	...
	private void initHistoryIds(SOSHibernateSession sosHibernateSession) throws SOSHibernateException {
		...
		filterJobStreamHistory.setRunning(true);
		DBLayerJobStreamHistory dbLayerJobStreamHistory = new DBLayerJobStreamHistory(sosHibernateSession);
		List<DBItemJobStreamHistory> listOfJobStreamHistory = dbLayerJobStreamHistory
				.getJobStreamHistoryList(filterJobStreamHistory, 0);
		for (DBItemJobStreamHistory dbItemJobStreamHistory : listOfJobStreamHistory) {
			try {
				if (dbItemJobStreamHistory.getRunning()) {
					this.listOfHistoryIds.put(UUID.fromString(dbItemJobStreamHistory.getContextId()),
							dbItemJobStreamHistory);
				}
	...
	private void initJobStreams(SOSHibernateSession sosHibernateSession)
			throws JsonParseException, JsonMappingException, JsonProcessingException, SOSInvalidDataException,
			DOMException, SOSHibernateException, IOException, ParseException, TransformerException {
		...
		jsJobStreams.setListOfJobStreams(settings, listOfJobStreams, listOfJobStreamStarter, listOfCalendarUsages,
				sosHibernateSession);

	}

	public void initJobOutConditions(SOSHibernateSession sosHibernateSession, String jobStream)
		...
		List<DBItemOutConditionWithConfiguredEvent> listOfOutConditions = dbLayerOutConditions
	...
	private void init(SOSHibernateSession sosHibernateSession, boolean complete)
			throws JsonParseException, JsonMappingException, JsonProcessingException, IOException,
			SOSHibernateException, SOSInvalidDataException, DOMException, ParseException, TransformerException {
		...
		if (jsEvents == null) {
			...
			for (JSInConditions jsInConditions : jsJobStreamInConditions.getListOfJobStreamInConditions().values()) {
				for (JSInCondition jsInCondition : jsInConditions.getListOfInConditions().values()) {
					List<JSCondition> listOfConditions = JSConditions
							.getListOfConditions(jsInCondition.getExpression());
					for (JSCondition jsCondition : listOfConditions) {
						if (jsCondition.isNonContextEvent()) {
							...
							List<DBItemOutConditionWithEvent> listOfNonContextEvents = dbLayerEvents
									.getEventsList(filterEvents, 0);
		...
		for (Map.Entry<Long, JSJobStream> jobStream : jsJobStreams.getListOfJobStreams().entrySet()) {
			...
			if (jobStream.getValue().getListOfJobStreamHistory() != null) {
				for (JSHistoryEntry jsHistoryEntry : jobStream.getValue().getListOfJobStreamHistory()) {
					...
					List<DBItemConsumedInCondition> listOfConsumedInConditions = dbLayerCoumsumedInConditions
							.getConsumedInConditionsList(filterConsumedInConditions, 0);
					...
					for (JSInConditions jobInConditions : jsJobInConditions.getListOfJobInConditions().values()) {
						for (JSInCondition inCondition : jobInConditions.getListOfInConditions().values()) {
							if (inCondition.isConsumed(contextId)
									&& mapOfConsumedInCondition.get(inCondition.getId()) == null) {
							...
							if (!inCondition.isConsumed(contextId)
								...
								if (isTraceEnabled) {
									LOGGER.trace(
											String.format("Consumed In Condition deleted %s", inCondition.toStr()));
								}
							}
						}
					}
			...
			for (JSInCondition inCondition : jobInConditions.getListOfInConditions().values()) {
				...
				for (JSCondition jsCondition : listOfConditions) {
					try {
						...
						case JOB: {
							checkHistoryCondition.validateJob(sosHibernateSession, jsCondition, inCondition.getJob(),
									0);
							break;
						}
						case JOB_CHAIN: {
						...
						LOGGER.warn("Could not validate expression:" + expressionValue + " in In Condition of job "
								+ inCondition.getJob());
					}
				}
			}
			...
			for (JSOutCondition outCondition : jobOutConditions.getListOfOutConditions().values()) {
				...
				for (JSCondition jsCondition : listOfConditions) {
					try {
						...
						case JOB: {
							checkHistoryCondition.validateJob(sosHibernateSession, jsCondition, outCondition.getJob(),
									0);
							break;
						}
						case JOB_CHAIN: {
	...
	private String expressionPrepare(String e) {
		return "<###" + e.replaceAll("\\(", "###(###").replaceAll("\\)", "###)###").replaceAll(" (?i)and ", "###&&&###")
				.replaceAll(" (?i)or ", "###|||###").replaceAll("(?i)not ", "###!!!###") + "###>";
	...
	public boolean validate(SOSHibernateSession sosHibernateSession, Integer taskReturnCode, UUID contextId,
			IJSCondition condition) {
		...
		for (JSCondition jsCondition : listOfConditions) {
			...
			case "returncode": {
				...
				if (returnCodeResolver.resolve(taskReturnCode, jsCondition.getConditionParam())) {
					...
					expressionValue = expressionValue.replace("###" + jsCondition.getConditionValue() + "###",
							"###true###");
			...
			case JOB: {
				try {
					if (checkHistoryCondition
							.validateJob(sosHibernateSession, jsCondition, condition.getJob(), taskReturnCode)
							.getValidateResult()) {
						expressionValue = this.expressionPrepare(expressionValue);
						expressionValue = expressionValue.replace("###" + jsCondition.getConditionValue() + "###",
								"###true###");
			...
			case "event": {
				...
				try {
					if (eventDate.isPrev(date)) {
						...
						date = String.valueOf(checkHistoryValue.getStartTime().getMonthValue()) + "."
								+ String.valueOf(checkHistoryValue.getStartTime().getDayOfMonth());
					}
				} catch (Exception e) {
				...
				if (jsCondition.typeIsGlobalEvent()) {
					...
					LOGGER.debug("Global: setting session to: " + session);
				...
				if (jsCondition.typeIsGlobalEvent() || jsCondition.isHaveDate()
						|| (jsEventKey.getJobStream() != null && !"".equals(jsCondition.getConditionJobStream()))) {
					String session = eventDate.getEventDate(date);
	...
	public void handleStartedJob(UUID contextId, SOSHibernateSession sosHibernateSession, StartJobReturn startJobReturn,
			JSInCondition inCondition) throws SOSHibernateException {
		if (!startJobReturn.getStartedJob().isEmpty() && startJobReturn.isStarted()) {
			...
			jobStreamContexts.addTaskToContext(contextId, settings.getSchedulerId(), jobStarterOptions,
					sosHibernateSession);
			this.disableInconditionsForJob(settings.getSchedulerId(), startJobReturn.getStartedJob(), contextId);
		}
	}

	public List<JSInCondition> resolveInConditions(SOSHibernateSession sosHibernateSession, UUID instance)
		...
		for (Map.Entry<Long, JSJobStream> jobStream : jsJobStreams.getListOfJobStreams().entrySet()) {
			...
			if (jobStream.getValue() != null && jobStream.getValue().getListOfJobStreamHistory() != null) {
				...
				for (JSHistoryEntry jsHistoryEntry : jobStream.getValue().getListOfJobStreamHistory()) {
					...
					LOGGER.debug(String.format("Running JobStream: %s with contextId %s found",
							jsHistoryEntry.getItemJobStreamHistory().getJobStream(), jsHistoryEntry.getContextId()));
					if (jsJobInConditions != null && jsJobInConditions.getListOfJobInConditions().size() == 0) {
						...
						try {
							for (JSInConditions jobInConditions : jsJobInConditions.getListOfJobInConditions()
									.values()) {
								...
								for (JSInCondition inCondition : jobInConditions.getListOfInConditions().values()) {
									...
									if (isTraceEnabled) {
										logPrompt = "job: " + inCondition.getJob() + " Job Stream: "
												+ inCondition.getJobStream() + " Expression: "
									...
									JSJobStream jsJobStream = this.getJsJobStreams()
											.getJobStream(jsHistoryEntry.getItemJobStreamHistory().getJobStream());
									...
									if (!endedByJob) {
										...
										if (validate(sosHibernateSession, null, contextId, inCondition)) {
											StartJobReturn startJobReturn = inCondition.executeCommand(
													sosHibernateSession, contextId, listOfParameters.get(contextId),
													schedulerXmlCommandExecutor);

											handleStartedJob(contextId, sosHibernateSession, startJobReturn,
													inCondition);
											inCondition.setEvaluatedContextId(contextId);
											LOGGER.debug("Adding in condition: " + inCondition.asString());
	...
	public CheckRunningResult checkRunning(SOSHibernateSession sosHibernateSession, UUID contextId)
			throws SOSHibernateException {
		...
		for (Map.Entry<Long, JSJobStream> jobStream : jsJobStreams.getListOfJobStreams().entrySet()) {
			if (jobStream.getValue() != null && jobStream.getValue().getListOfJobStreamHistory() != null) {
				...
				for (JSHistoryEntry jsHistoryEntry : jobStream.getValue().getListOfJobStreamHistory()) {
					if (jsHistoryEntry.getContextId().toString().equals(contextId.toString())
							&& jsHistoryEntry.isRunning()) {
						LOGGER.debug(String.format("Running JobStream: %s with contextId %s found",
								jsHistoryEntry.getItemJobStreamHistory().getJobStream(),
								jsHistoryEntry.getContextId()));
						if (!jsHistoryEntry.checkRunning(this)) {
							try {
								JSJobStream jsJobStream = getJsJobStreams()
										.getJobStream(jsHistoryEntry.getItemJobStreamHistory().getJobStream());
								...
								DBLayerJobStreamHistory dbLayerJobStreamHistory = new DBLayerJobStreamHistory(
										sosHibernateSession);
								DBItemJobStreamHistory dbItemJobStreamHistory = dbLayerJobStreamHistory
										.getJobStreamHistoryDbItem(jsHistoryEntry.getId());

								FilterConsumedInConditions filterConsumedInConditions = new FilterConsumedInConditions();
								filterConsumedInConditions.setJobSchedulerId(jsJobStream.getJobSchedulerId());
								filterConsumedInConditions.setJobStream(resolveOutConditionResult.getJobStream());
								filterConsumedInConditions.setSession(contextId.toString());
								removeConsumedInconditions(sosHibernateSession, filterConsumedInConditions);

								if (this.listOfHistoryIds.get(contextId) != null) {
	...
	public boolean resolveOutConditions(SOSHibernateSession sosHibernateSession, TaskEndEvent taskEndEvent,
			String jobSchedulerId, String job) throws SOSHibernateException {
				...
				if (jobOutConditions != null) {
					for (JSOutCondition outCondition : jobOutConditions.getListOfOutConditions().values()) {
						...
						if (validate(sosHibernateSession, taskEndEvent.getReturnCode(), contextId, outCondition)) {
							...
							dbChanged = outCondition.storeOutConditionEvents(sosHibernateSession, contextId.toString(),
									historyEntry, jsEvents, newJsEvents, removeJsEvents);
	...
	public void setJobIsRunningInconditionsForJob(boolean value, String jobSchedulerId, String job, UUID contextId)
			throws SOSHibernateException {
	...
	public void enableInconditionsForJob(String jobSchedulerId, String job, UUID contextId)
			throws SOSHibernateException {
		setJobIsRunningInconditionsForJob(false, jobSchedulerId, job, contextId);
	}

	public void disableInconditionsForJob(String jobSchedulerId, String job, UUID contextId)
			throws SOSHibernateException {
	...
	private void removeConsumedInconditions(SOSHibernateSession sosHibernateSession,
			FilterConsumedInConditions filterConsumedInConditions) throws SOSHibernateException {
		...
		try {
			...
			/*
			 * try { DBLayerConsumedInConditions dbLayerConsumedInConditions = new
			 * DBLayerConsumedInConditions(sosHibernateSession);
			 * sosHibernateSession.beginTransaction();
			 * dbLayerConsumedInConditions.deleteConsumedInConditions(
			 * filterConsumedInConditions); sosHibernateSession.commit(); } catch (Exception
			 * e) { LOGGER.error(e.getMessage(), e); sosHibernateSession.rollback(); }
			 */
			for (JSInConditions jobInConditions : jsJobInConditions.getListOfJobInConditions().values()) {
				for (JSInCondition inCondition : jobInConditions.getListOfInConditions().values()) {
					if ((filterConsumedInConditions.getJobStream().equals(inCondition.getJobStream())
							|| filterConsumedInConditions.getJobStream().isEmpty())
							&& (filterConsumedInConditions.getJob().equals(inCondition.getJob())
									|| filterConsumedInConditions.getJob().isEmpty())
							&& inCondition.isConsumed(contextId)) {
	...
	public void checkHistoryCache(SOSHibernateSession sosHibernateSession, String jobPath, Integer taskReturnCode)
			throws Exception {
		...
		if (listOfCheckHistoryChacheRules == null) {
			...
			for (CheckHistoryCacheRule checkHistoryCacheRule : listOfCheckHistoryChacheRules
					.getListOfCheckHistoryChacheRules()) {
				...
				if (validateResult != null && ((checkHistoryCacheRule.isValidateAlways())
						|| (checkHistoryCacheRule.isValidateIfFalse() && !validateResult.getValidateResult()))) {
					checkHistoryCondition.putCache(checkHistoryKey, null);
					checkHistoryCondition.validateJob(sosHibernateSession, validateResult.getJsCondition(), jobPath,
							taskReturnCode);
				}
			}
		}
	...
	public void assign(SOSHibernateSession sosHibernateSession, JSConditionResolver lastConditionResolver)
			throws SOSHibernateException, JsonParseException, JsonMappingException, JsonProcessingException,
			SOSInvalidDataException, DOMException, IOException, ParseException, TransformerException {
		...
		if (jsOutConditions != null) {
			for (Entry<Long, JSOutCondition> entry : jsOutConditions.getListOfOutConditions().entrySet()) {
				...
				for (JSOutConditionEvent jsOutConditionEvent : jsOutCondition.getListOfOutConditionEvent()) {
					if (jsOutConditionEvent.getCommand().endsWith("create")
							&& jsOutConditionEvent.getEvent().equals(event)) {
