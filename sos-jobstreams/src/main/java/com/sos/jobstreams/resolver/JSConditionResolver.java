...
public class JSConditionResolver {
	...
	public JSConditionResolver(SOSHibernateSession sosHibernateSession,
			SchedulerXmlCommandExecutor schedulerXmlCommandExecutor, EventHandlerSettings settings) {
		super();
	...
	public void init()
		...
		if (jsJobStreams == null) {
			...
			/*
			 * sosHibernateSession.beginTransaction(); LOGGER.debug("timeZone:" +
			 * TimeZone.getDefault()); DBLayerJobStreamStarters dbLayerJobStreamStarters =
			 * new DBLayerJobStreamStarters(sosHibernateSession); for (Map.Entry<Long,
			 * JSJobStreamStarter> entry : listOfJobStreamStarter.entrySet()) { Date
			 * nextStart = entry.getValue().getNextStartFromList(); if (nextStart != null) {
			 * LOGGER.debug("next Start:" + new Date(nextStart.getTime())); DateTime
			 * nextDateTime = new DateTime(nextStart.getTime()); DBItemJobStreamStarter
			 * dbItemJobStreamStarter = entry.getValue().getItemJobStreamStarter(); Calendar
			 * calendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
			 * calendar.setTimeInMillis(UtcTimeHelper.convertTimeZonesToDate(UtcTimeHelper.
			 * localTimeZoneString(), "UTC", nextDateTime).getTime());
	...
	public boolean validate(Integer taskReturnCode, UUID contextId, IJSCondition condition) {
	...
	public List<JSInCondition> resolveInConditions(SOSHibernateSession session, String jobSchedulerId)
		...
		for (Map.Entry<Long, JSJobStream> jobStream : jsJobStreams.getListOfJobStreams().entrySet()) {
			...
			if (jobStream.getValue() != null && jobStream.getValue().getListOfJobStreamHistory() != null) {
				...
				if (jobStream.getValue() != null) {
					...
					for (Iterator<JSHistoryEntry> iterator = jobStream.getValue().getListOfJobStreamHistory()
										...
										if (!(inCondition.isConsumed(contextId) && inCondition.isMarkExpression())) {
											if (!inCondition.jobIsRunning(contextId)) {
												...
												if (validate(null, contextId, inCondition)) {
													...
													StartJobReturn startJobReturn = inCondition.executeCommand(session,
															contextId, listOfParameters.get(contextId),
													...
													if (!startJobReturn.getStartedJob().isEmpty()) {
														...
														jobStreamContexts.addTaskToContext(contextId, jobSchedulerId,
																jobStarterOptions, session);
														this.disableInconditionsForJob(settings.getSchedulerId(),
																startJobReturn.getStartedJob(), contextId);
													}
													inCondition.setEvaluatedContextId(contextId);
													LOGGER.debug(
															"Adding in condition: " + SOSString.toString(inCondition));
			...
			if (contextId != null) {
				...
				if (jobOutConditions != null) {
					for (JSOutCondition outCondition : jobOutConditions.getListOfOutConditions().values()) {
						...
						if (validate(taskEndEvent.getReturnCode(), contextId, outCondition)) {
							LOGGER.trace("create/remove events ------>");
							DBItemJobStreamHistory historyEntry = listOfHistoryIds.get(contextId);
							dbChange = outCondition.storeOutConditionEvents(sosHibernateSession, contextId.toString(),
									historyEntry, jsEvents, newJsEvents, removeJsEvents);
							outCondition.storeOutConditionEvents(sosHibernateSession, defaultSession, historyEntry,
									jsEvents, newJsEvents, removeJsEvents);
						} else {
				...
				if (this.listOfHistoryIds.get(contextId) != null
						&& !this.listOfHistoryIds.get(contextId).getRunning()) {
	...
	public void removeConsumedInconditions(FilterConsumedInConditions filterConsumedInConditions)
			throws SOSHibernateException {
	...
	public void removeEventsFromJobStream(FilterEvents filter) throws SOSHibernateException {
		LOGGER.debug(
				"JSConditionResolver::removeEventsFromJobStream --> " + filter.getJobStream() + "." + filter.getJob());
		try {
