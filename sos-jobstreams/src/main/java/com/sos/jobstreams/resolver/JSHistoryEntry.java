...
public class JSHistoryEntry {
    ...
    public EndOfJobStreamReturn endOfStream(JSJobStream jsJobStream, Long starterId, JobStreamContexts jobStreamContexts) {
        // Checking all starters whether there is an end job
        LOGGER.debug("Check for end job in job stream: " + jsJobStream.getJobStream());
        EndOfJobStreamReturn endOfStreamReturn = new EndOfJobStreamReturn();
        boolean endReached = false;
        for (JSJobStreamStarter jsJobStreamStarter : jsJobStream.getListOfJobStreamStarter()) {
            if (jsJobStreamStarter.getItemJobStreamStarter().getId().equals(starterId)) {
                if (!"".equals(jsJobStreamStarter.getEndJob()) && jsJobStreamStarter.getEndJob() != null) {
                    LOGGER.debug("end job found for starter: " + jsJobStreamStarter.getItemJobStreamStarter().getStarterName() + " Job: "
                            + jsJobStreamStarter.getEndJob());
                    ...
                    if (listOfJobStarterOptions != null) {
                        for (JobStarterOptions jobStarterOptions : listOfJobStarterOptions) {
                            ...
                            if (this.getContextId().toString().equals(jobStarterOptions.getInstanceId())) {
                                LOGGER.debug("instance " + jobStarterOptions.getInstanceId() + " have an endJob " + jsJobStreamStarter.getEndJob());
                                endOfStreamReturn.haveEndJob = true;
                            }
        ...
        endOfStreamReturn.endReached = endReached;
        return endOfStreamReturn;
    }
    ...
    public boolean checkRunning(JSConditionResolver jsConditionResolver) {
        ...
        // If a lastJob is defined, then the lastJob inCondition must be consumed.
        ...
        Long starterId = 0L;
        if ((jsConditionResolver.getListOfHistoryIds() != null) && (jsConditionResolver.getListOfHistoryIds().get(this.getContextId()) != null)) {
            starterId = jsConditionResolver.getListOfHistoryIds().get(this.getContextId()).getJobStreamStarter();
        }

        LOGGER.debug("- JobStream: " + jsJobStream.getJobStream());
        LOGGER.debug("- JobStreamStarterId: " + starterId);
        ...
        if (jsInConditions != null && jsInConditions.getListOfInConditions() != null) {
            ...
            EndOfJobStreamReturn endOfStreamReturn = this.endOfStream(jsJobStream, starterId, jobStreamContexts);
            if (endOfStreamReturn.haveEndJob) {
                return !endOfStreamReturn.endReached;
            } else {
                if (!endOfStreamReturn.endReached) {
            ...
            if (!running && jsInConditions != null && jsInConditions.getListOfInConditions() != null && jsInConditions.getListOfInConditions()
                    .values() != null) {
                for (JSInCondition jsInCondition : jsInConditions.getListOfInConditions().values()) {
                    LOGGER.debug("- InCondition expression : " + jsInCondition.getExpression());
