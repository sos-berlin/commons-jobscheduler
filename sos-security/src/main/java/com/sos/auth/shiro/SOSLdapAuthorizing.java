...
public class SOSLdapAuthorizing {
    ...
    private Collection<String> getGroupNamesByUser() throws NamingException {
        ...
        LOGGER.debug(String.format("getting groups from ldap using user search filter %s with search base %s", searchFilter, sosLdapAuthorizingRealm
                .getSearchBase()));
    ...
    public void setSosLdapAuthorizingRealm(SOSLdapAuthorizingRealm sosLdapAuthorizingRealm) throws IOException, NamingException {
        ...
        try {
            if (jndiLdapContextFactory.getSystemUsername() != null && !jndiLdapContextFactory.getSystemUsername().isEmpty()) {
            ...
            if (sosLdapAuthorizingRealm.isUseStartTls_()) {
                ...
                if (jndiLdapContextFactory.getSystemUsername() != null && !jndiLdapContextFactory.getSystemUsername().isEmpty()) {
                    ldapContext.addToEnvironment(Context.SECURITY_PRINCIPAL, jndiLdapContextFactory.getSystemUsername());
                    ldapContext.addToEnvironment(Context.SECURITY_CREDENTIALS, jndiLdapContextFactory.getSystemPassword());
                } else {
