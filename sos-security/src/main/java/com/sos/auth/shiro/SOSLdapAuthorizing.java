...
public class SOSLdapAuthorizing {
	...
	public SimpleAuthorizationInfo setRoles(SimpleAuthorizationInfo authorizationInfo_,
	...
	private Attributes getUserAttributes() throws NamingException {
		...
		LOGGER.debug(String.format("getting user from ldap using search filter %s with search base %s",
				sosLdapAuthorizingRealm.getSearchBase(), searchFilter));
		...
		if (!answer.hasMore()) {
			LOGGER.warn(String.format("Cannot find user: %s with search filter %s and search base: %s ",
					sosLdapLoginUserName.getLogin(), searchFilter, searchBase));
		} else {
	...
	private Collection<String> getGroupNamesByUser() throws NamingException {
		...
		NamingEnumeration<SearchResult> answer = ldapContext.search(sosLdapAuthorizingRealm.getSearchBase(),
				searchFilter, searchCtls);

		if (!answer.hasMore()) {
			LOGGER.warn(String.format("Cannot find roles for user: %s with search filter %s and search base: %s ",
					sosLdapLoginUserName.getUserName(), searchFilter, sosLdapAuthorizingRealm.getSearchBase()));
		} else {
			SearchResult result = answer.nextElement();
			String groupNameAttribute = sosLdapAuthorizingRealm.getGroupNameAttribute();
	...
	private void getRoleNamesForUser() throws Exception {
		...
		if (s != null && sosLdapAuthorizingRealm.isRoleAssignmentFromIni()) {
			...
			if (!addRolesFromUserSection(caseInsensitivUser, sosLdapLoginUserName.getLogin())) {
				LOGGER.debug("... not found: reading roles for " + sosLdapLoginUserName.getLogin()
						+ " from section [users]");
				if (sosLdapLoginUserName.getAlternateLogin() != null) {
					if (!addRolesFromUserSection(caseInsensitivUser, sosLdapLoginUserName.getAlternateLogin()))
						LOGGER.debug("... not found: reading roles for " + sosLdapLoginUserName.getAlternateLogin()
								+ " from section [users]");
				}
			}
		}

		if (sosLdapAuthorizingRealm.getUserNameAttribute() != null
				&& !sosLdapAuthorizingRealm.getUserNameAttribute().isEmpty()) {
			...
			if (user != null) {
				if (user.get(sosLdapAuthorizingRealm.getUserNameAttribute()) == null) {
					...
					sosLdapLoginUserName
							.setUserName(user.get(sosLdapAuthorizingRealm.getUserNameAttribute()).get().toString());
				}
			} else {
		...
		if (sosLdapAuthorizingRealm.isGetRolesFromLdap()) {
			if ((sosLdapAuthorizingRealm.getSearchBase() != null
					...
					&& (sosLdapAuthorizingRealm.getGroupSearchFilter() != null
							|| sosLdapAuthorizingRealm.getUserSearchFilter() != null)) {

				Collection<String> groupNames;

				if (sosLdapAuthorizingRealm.getGroupSearchFilter() != null
						&& !sosLdapAuthorizingRealm.getGroupSearchFilter().isEmpty()) {
	...
	public void getRoleNamesForUserTest(SOSLdapAuthorizingRealm sosLdapAuthorizingRealm, String username)
			throws Exception {
		...
		try {
			...
			if (sosLdapAuthorizingRealm.isUseStartTls()) {
				...
				if (Globals.withHostnameVerification) {
					if ("false".equalsIgnoreCase(sosLdapAuthorizingRealm.getHostNameVerification())
							|| "off".equalsIgnoreCase(sosLdapAuthorizingRealm.getHostNameVerification())) {
					...
					if ("true".equalsIgnoreCase(sosLdapAuthorizingRealm.getHostNameVerification())
							|| "on".equalsIgnoreCase(sosLdapAuthorizingRealm.getHostNameVerification())) {
						LOGGER.debug("HostNameVerification=true");
					} else {
						LOGGER.debug("HostNameVerification=false");
						tls.setHostnameVerifier(new DummyVerifier());
