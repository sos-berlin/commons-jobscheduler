...
public class SOSServicePermissionShiro {
	...
	public JOCDefaultResponse postJocCockpitPermissions(@HeaderParam(ACCESS_TOKEN) String accessTokenFromHeader) {
		...
		try {
			...
			setCurrentUserfromAccessToken(sosWebserviceAuthenticationRecord.getAccessToken(),
					sosWebserviceAuthenticationRecord.getUser(), sosWebserviceAuthenticationRecord.getPassword());

			if (currentUser == null) {
				LOGGER.debug(USER_IS_NULL + " " + AUTHORIZATION_HEADER_WITH_BASIC_BASED64PART_EXPECTED);
				return JOCDefaultResponse.responseStatusJSError(
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public JOCDefaultResponse loginPost(@HeaderParam("Authorization") String basicAuthorization,
	...
	private void setCurrentUserfromAccessToken(String accessToken, String user, String pwd) {
		if (Globals.jocWebserviceDataContainer.getCurrentUsersList() != null && accessToken != null
			...
			if (user != null && user.length() > 0 && pwd != null && pwd.length() > 0) {
				LOGGER.debug(String.format("Method: %s, User: %s, access_token: %s", "setCurrentUserfromAccessToken",
						user, accessToken));
	...
	public SOSPermissionShiro getPermissions(@HeaderParam(ACCESS_TOKEN) String accessTokenFromHeader,
		...
		if (currentUser != null && currentUser.getCurrentSubject() != null) {
			...
			addPermission(sosPermissionJoc.getSOSPermission(),
			...
			addPermission(sosPermissionJoc.getSOSPermission(),
					"sos:products:joc_cockpit:customization:private:make_shared");
			addPermission(sosPermissionJoc.getSOSPermission(), "sos:products:joc_cockpit:customization:private:delete");
			addPermission(sosPermissionJoc.getSOSPermission(),
					"sos:products:joc_cockpit:customization:private:edit_content");
			...
			addPermission(sosPermissionCommands.getSOSPermission(),
					"sos:products:commands:jobscheduler_master:view:status");
			addPermission(sosPermissionCommands.getSOSPermission(),
					"sos:products:commands:jobscheduler_master:view:calendar");
			addPermission(sosPermissionCommands.getSOSPermission(),
					"sos:products:commands:jobscheduler_master:view:parameter");
			addPermission(sosPermissionCommands.getSOSPermission(),
					"sos:products:commands:jobscheduler_master:execute:restart:terminate");
			...
			addPermission(sosPermissionCommands.getSOSPermission(),
					"sos:products:commands:job_chain:change:hot_folder");
	...
	private SOSPermissionJocCockpit getSosPermissionJocCockpit() {
		...
		if (currentUser != null && currentUser.getCurrentSubject() != null) {
			...
			sosPermissionJocCockpit.getJobschedulerMaster()
					.setRestart(o.createSOSPermissionJocCockpitJobschedulerMasterRestart());
			...
			sosPermissionJocCockpit.getJobschedulerUniversalAgent()
					.setRestart(o.createSOSPermissionJocCockpitJobschedulerUniversalAgentRestart());
			...
			sosPermissionJocCockpit.getJobschedulerMaster().getRestart()
			...
			sosPermissionJocCockpit.getJobschedulerMaster()
					.setManageCategories(haveRight("sos:products:joc_cockpit:jobscheduler_master:manage_categories"));
			...
			sosPermissionJocCockpit.getJobschedulerMasterCluster().setTerminateFailSafe(
			...
			sosPermissionJocCockpit.getJobschedulerUniversalAgent().getRestart()
					.setAbort(haveRight("sos:products:joc_cockpit:jobscheduler_universal_agent:execute:restart:abort"));
			sosPermissionJocCockpit.getJobschedulerUniversalAgent().getRestart().setTerminate(
			...
			sosPermissionJocCockpit.getOrder()
					.setSetRunTime(haveRight("sos:products:joc_cockpit:order:change:run_time"));
			sosPermissionJocCockpit.getOrder().setSetState(haveRight("sos:products:joc_cockpit:order:change:state"));
			sosPermissionJocCockpit.getOrder()
					.setModifyHotFolder(haveRight("sos:products:joc_cockpit:order:change:hot_folder"));
			...
			sosPermissionJocCockpit.getJobChain()
			...
			sosPermissionJocCockpit.getJobChain().setProcessJobChainNode(
			...
			sosPermissionJocCockpit.getJob()
			...
			sosPermissionJocCockpit.getSchedule()
					.setAddSubstitute(haveRight("sos:products:joc_cockpit:schedule:add_substitute"));
			...
			sosPermissionJocCockpit.getEventAction().getView()
					.setStatus(haveRight("sos:products:joc_cockpit:event_action:view:status"));
			sosPermissionJocCockpit.getEventAction()
					.setCreateEventsManually(haveRight("sos:products:joc_cockpit:event_action:create_event_manually"));
			...
			sosPermissionJocCockpit.getJOCConfigurations().getShare()
					.setView(haveRight("sos:products:joc_cockpit:customization:share:view"));
			sosPermissionJocCockpit.getJOCConfigurations().getShare().getChange()
	...
	private SOSPermissionCommands getSosPermissionCommands() {
		...
		if (currentUser != null && currentUser.getCurrentSubject() != null) {
			...
			sosPermissionCommands.getJobschedulerMaster()
					.setRestart(o.createSOSPermissionCommandsJobschedulerMasterRestart());
			...
			sosPermissionCommands.getJobschedulerMaster().getRestart()
			...
			sosPermissionCommands.getJobschedulerMasterCluster().setTerminateFailSafe(
			...
			sosPermissionCommands.getOrder()
					.setModifyHotFolder(haveRight("sos:products:commands:order:change:hot_folder"));
			...
			sosPermissionCommands.getJobChain()
			...
			sosPermissionCommands.getJobChain()
					.setModifyHotFolder(haveRight("sos:products:commands:job_chain:change:hot_folder"));
			...
			sosPermissionCommands.getProcessClass()
					.setChange(haveRight("sos:products:commands:process_class:edit:change"));
			sosPermissionCommands.getProcessClass()
					.setRemove(haveRight("sos:products:commands:process_class:edit:remove"));
			sosPermissionCommands.getProcessClass()
					.setModifyHotFolder(haveRight("sos:products:commands:process_class:modify_hot_folder"));

			sosPermissionCommands.getSchedule().getView()
					.setStatus(haveRight("sos:products:commands:schedule:view:status"));
			sosPermissionCommands.getSchedule()
					.setAddSubstitute(haveRight("sos:products:commands:schedule:add_substitute"));
			sosPermissionCommands.getSchedule()
					.setModifyHotFolder(haveRight("sos:products:commands:schedule:modify_hot_folder"));
			...
			sosPermissionCommands.getLock()
					.setModifyHotFolder(haveRight("sos:products:commands:lock:modify_hot_folder"));
		}
	...
	private SOSShiroCurrentUser getUserPwdFromHeaderOrQuery(String basicAuthorization, String user, String pwd)
		...
		if (basicAuthorization != null) {
			...
			if (authorizationParts.length > 1) {
				authorization = new String(Base64.getDecoder().decode(authorizationParts[1].getBytes("UTF-8")),
						"UTF-8");
			}
		} else {
			JocError error = new JocError();
	...
	private JOCDefaultResponse login(String basicAuthorization, String user, String pwd) {
		...
		try {
			...
			LOGGER.debug(String.format("Method: %s, User: %s, access_token: %s", "login", currentUser.getUsername(),
