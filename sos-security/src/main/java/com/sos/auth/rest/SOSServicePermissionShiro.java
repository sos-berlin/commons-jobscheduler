...
public class SOSServicePermissionShiro {
	...
	public JOCDefaultResponse getSize() {
		if (Globals.jocWebserviceDataContainer.getCurrentUsersList() == null) {
			...
			return JOCDefaultResponse
					.responseStatus200(Globals.jocWebserviceDataContainer.getCurrentUsersList().size());
		}
	}
	...
	public JOCDefaultResponse getAccessToken(@HeaderParam(ACCESS_TOKEN) String accessTokenFromHeader,
		...
		if (Globals.jocWebserviceDataContainer.getCurrentUsersList() != null) {
			...
			SOSShiroCurrentUserAnswer s = Globals.jocWebserviceDataContainer.getCurrentUsersList()
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public JOCDefaultResponse loginPost(@Context HttpServletRequest request,
			@HeaderParam("Authorization") String basicAuthorization, @QueryParam("user") String user,
	...
	private void removeTimeOutSessions()
			throws SOSHibernateException, JocConfigurationException, DBOpenSessionException {
		...
		try {
			...
			List<JocConfigurationDbItem> listOfConfigurtions = jocConfigurationDBLayer.getJocConfigurationList(filter,
					0);
	...
	private String getAccessToken(String accessTokenFromHeader, String xAccessTokenFromHeader,
			String accessTokenFromQuery) {
	...
	private void createUser() throws Exception {
		...
		if (sosLogin.getCurrentUser() == null) {
			...
			sosShiroCurrentUserAnswer.setIsAuthenticated(false);
			sosShiroCurrentUserAnswer.setMessage(String.format("%s: Could not login with user: %s password:*******",
					sosLogin.getMsg(), currentUser.getUsername()));
		...
		currentUser.getCurrentSubject().getSession().setAttribute("username_joc_permissions",
				SOSSerializerUtil.object2toString(sosPermissionJocCockpitMasters));
		...
		currentUser.getCurrentSubject().getSession().setAttribute("username_command_permissions",
				SOSSerializerUtil.object2toString(sosPermissionCommandsMasters));
	...
	private JOCDefaultResponse login(HttpServletRequest request, String basicAuthorization, String user, String pwd)
